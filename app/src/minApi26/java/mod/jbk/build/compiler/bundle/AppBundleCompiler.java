...
public class AppBundleCompiler {
  ...
  public void buildBundle() throws zy {
                ...
                Config.BundleConfig.newBuilder()
                    .setCompression(
                        Config.Compression.newBuilder()
                            .addAllUncompressedGlob(uncompressedModuleMainPaths)
                            .build())
                    .build());
    if (builder.proguard.isShrinkingEnabled() && builder.proguard.isDebugFilesEnabled()) {
      ...
      LogUtil.d(
          ...
          "Adding metadata file "
              + mapping
              + " as com.android.tools.build.obfuscation/proguard.map");
    ...
    LogUtil.d(
        TAG, "Building app bundle took " + (System.currentTimeMillis() - savedTimeMillis) + " ms");
  }
  ...
  public void createModuleMainArchive() throws IOException {
    ...
    try (var moduleMainZip = new ZipOutputStream(moduleMain)) {
      ...
      var nativeLibrariesDirectory =
          new File(new FilePathUtil().getPathNativelibs(builder.yq.sc_id));
        ...
        try (var jarStream = new FileInputStream(jar);
          ...
          while (jarArchiveEntry != null) {
            ...
            if (!jarArchiveEntry.isDirectory()
                && !pathInJar.equals("META-INF/MANIFEST.MF")
                && !pathInJar.endsWith(".class")) {
              var nonClassFileToAddToModule =
                  new ZipEntry(MODULE_ROOT + File.separator + pathInJar);
