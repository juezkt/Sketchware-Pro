...
public class KotlinCompilerUtil {
   ...
   *
   * <p>> .sketchware/mysc/xxx/app/src/main/java, > .sketchware/mysc/xxx/gen, >
  ...
  public static boolean areAnyKtFilesPresent(yq yq) {
    return getFilesToCompile(yq).stream().anyMatch(it -> it.getName().endsWith(".kt"));
  }

  /**
   * Returns a list of `.java` & `.kt` files from the specified dirs.
   *
   * <p>The name might be a little misleading, but only `.kt` files are actually compiled by
   * kotlinc, `.java` files are used for classpath. (for Java-Kotlin interoperability)
   */
  static List<File> getFilesToCompile(yq workspace) {
    ...
    // .sketchware/data/xxx/files/java
    mFilesToCompile.addAll(getSourceFiles(new File(new FilePathUtil().getPathJava(scId))));

    return mFilesToCompile;
  }

  /**
   * Returns a list of available kotlin compiler plugins (.jar) found in
   * `/.sketchware/data/xxx/files/kt_plugins` dir.
   */
