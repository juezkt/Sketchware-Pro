...
public class ManageResourceActivity extends BaseAppCompatActivity {
          ...
          for (String path : selections) {
            try {
              FileUtil.copyDirectory(
                  new File(path),
                  new File(temp + File.separator + Uri.parse(path).getLastPathSegment()));
            } catch (IOException e) {
  ...
  private void showDeleteDialog(final int position) {
    new MaterialAlertDialogBuilder(this)
        .setTitle(
            "Delete "
                + Uri.fromFile(new File(adapter.getItem(position))).getLastPathSegment()
                + "?")
        .setMessage(
                    ...
                    case "Edit with..." -> {
                      if (frc.listFileResource.get(position).endsWith("xml")) {
                        ...
                        intent.setDataAndType(
                            Uri.fromFile(new File(frc.listFileResource.get(position))),
                            "text/plain");
                        startActivity(intent);
                      } else {
                        SketchwareUtil.toast("Only XML files can be edited");
