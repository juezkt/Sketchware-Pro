...
public class ManagePermissionActivity extends BaseAppCompatActivity {
        ...
        item -> {
          if (item.getItemId() == R.id.action_reset) {
            new MaterialAlertDialogBuilder(this)
                ...
                .setMessage(
                    "Are you sure you want to reset all permissions? This cannot be undone!")
                .setPositiveButton(
                    "Reset",
                    (dialog, which) -> {
                      FileUtil.writeFile(new FilePathUtil().getPathPermission(numProj), "[]");
                      // As FileResConfig only refreshes permissions during <init>()V, this is
                      // required.
                      frc = new FileResConfig(numProj);
                      setItems();
                    })
  ...
  public void onBackPressed() {
    FileUtil.writeFile(
        new FilePathUtil().getPathPermission(numProj), new Gson().toJson(frc.getPermissionList()));
    super.onBackPressed();
  }
  ...
  private class PermissionsAdapter extends RecyclerView.Adapter<PermissionsAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
      var listBinding =
          ViewItemPermissionBinding.inflate(
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
      ...
      binding.checkboxContent.setOnCheckedChangeListener(
          (button, checked) -> {
            if (checked) {
