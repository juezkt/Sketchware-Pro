  ...
  public ManageLocalLibrary(String sc_id) {
    ...
    if (FileUtil.isExistFile(localLibraryConfigPath)) {
      try {
        ...
        if (list == null) {
          LogUtil.w(
              ...
              "Read null from file "
                  + localLibraryConfigPath
                  + ", deleting invalid configuration.");
      ...
      SketchwareUtil.toastError(
          "Invalid Local library configuration found! Temporarily using none");
    }
    list = new ArrayList<>();
  }
    ...
    for (int i = 0, listSize = list.size(); i < listSize; i++) {
      ...
      if (localLibrary.containsKey("assetsPath")) {
        ...
        if (assetsPath instanceof String) {
          ...
          SketchwareUtil.toastError(
              "Invalid assets path of enabled Local library #" + i, Toast.LENGTH_LONG);
        }
      }
    }
  ...
  public ArrayList<String> getDexLocalLibrary() {
    ...
    for (int i = 0, listSize = list.size(); i < listSize; i++) {
      ...
      if (dexPath instanceof String) {
        ...
        SketchwareUtil.toastError(
            "Invalid DEX path of enabled Local library #" + i, Toast.LENGTH_LONG);
      }
    }
  ...
  public ArrayList<String> getGenLocalLibrary() {
    ...
    for (String packageName : getPackageNames()) {
      if (!packageName.isEmpty()) {
        File projectGenFolder =
            new File(
                Environment.getExternalStorageDirectory(),
                ".sketchware/mysc/".concat(projectId).concat("/gen"));
        String rJavaPath =
            packageName.replace(".", File.separator).concat(File.separator).concat("R.java");
  ...
  public ArrayList<File> getLocalLibraryJars() {
    ...
    for (int i = 0, listSize = list.size(); i < listSize; i++) {
      ...
      if (jarPath instanceof String) {
        ...
        SketchwareUtil.toastError(
            "Invalid JAR path of enabled Local library #" + i + "->" + localLibrary.get("name"),
            Toast.LENGTH_LONG);
      }
    }
    ...
    for (int i = 0, listSize = list.size(); i < listSize; i++) {
      ...
      if (localLibrary.containsKey("packageName")) {
        ...
        if (packageName instanceof String) {
          ...
          SketchwareUtil.toastError(
              "Invalid package name of enabled Local library #" + i, Toast.LENGTH_LONG);
        }
      }
    }
    ...
    for (int i = 0, listSize = list.size(); i < listSize; i++) {
      ...
      if (localLibrary.containsKey("pgRulesPath")) {
        ...
        if (proguardRulesPath instanceof String) {
          ...
          SketchwareUtil.toastError(
              "Invalid ProGuard path of enabled Local library #" + i, Toast.LENGTH_LONG);
        }
      }
    }
    ...
    for (int i = 0, listSize = list.size(); i < listSize; i++) {
      ...
      if (localLibrary.containsKey("resPath")) {
        ...
        if (resPath instanceof String) {
          ...
          SketchwareUtil.toastError(
              "Invalid res/ folder path of enabled Local library #" + i, Toast.LENGTH_LONG);
        }
      }
    }
