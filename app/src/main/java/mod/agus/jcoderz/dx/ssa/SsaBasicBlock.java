...
public final class SsaBasicBlock {
   ...
   */
  public void addPhiInsnForReg(mod.agus.jcoderz.dx.rop.code.RegisterSpec resultSpec) {
   ...
   */
  public void addInsnToHead(mod.agus.jcoderz.dx.rop.code.Insn insn) {
   ...
   * the exit block, this block does nothing. For use by {@link SsaMethod#makeExitBlock}
   *
      ...
      NormalSsaInsn toAdd =
          new NormalSsaInsn(
              new mod.agus.jcoderz.dx.rop.code.PlainInsn(
                  mod.agus.jcoderz.dx.rop.code.Rops.opMove(result.getType()),
   ...
   * @param source move source
   */
  public void addMoveToBeginning(
      mod.agus.jcoderz.dx.rop.code.RegisterSpec result,
      mod.agus.jcoderz.dx.rop.code.RegisterSpec source) {
    if (result.getReg() == source.getReg()) {
      // Sometimes we end up with no-op moves. Ignore them here.
   ...
   */
  private static void setRegsUsed(BitSet regsUsed, mod.agus.jcoderz.dx.rop.code.RegisterSpec rs) {
   ...
   */
  private static boolean checkRegUsed(
      BitSet regsUsed, mod.agus.jcoderz.dx.rop.code.RegisterSpec rs) {
    int reg = rs.getReg();
  ...
  private void scheduleUseBeforeAssigned(List<SsaInsn> toSchedule) {
    ...
    while (insertPlace < sz) {
      ...
      if (oldInsertPlace == insertPlace) {
        ...
        mod.agus.jcoderz.dx.rop.code.RegisterSpec result = insnToSplit.getResult();
        mod.agus.jcoderz.dx.rop.code.RegisterSpec tempSpec =
            result.withReg(parent.borrowSpareRegister(result.getCategory()));
        ...
        mod.agus.jcoderz.dx.rop.code.RegisterSpecList newSources =
            mod.agus.jcoderz.dx.rop.code.RegisterSpecList.make(tempSpec);
      ...
      if (firstNonPhiMoveInsn.isMoveException()) {
        if (true) {
          ...
          if (!moveExceptionInterferes) {
             ...
             * and move it back.
             */
            mod.agus.jcoderz.dx.rop.code.RegisterSpec originalResultSpec =
