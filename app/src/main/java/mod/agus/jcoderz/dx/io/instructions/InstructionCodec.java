...
public enum InstructionCodec {
  ...
  FORMAT_20BC() {
    ...
    public DecodedInstruction decode(int opcodeUnit, CodeInput in) throws EOFException {
      ...
      int index = in.read();
      return new ZeroRegisterDecodedInstruction(
          this, opcode, index, mod.agus.jcoderz.dx.io.IndexType.VARIES, 0, literal);
    }
  ...
  FORMAT_21C() {
    ...
    public DecodedInstruction decode(int opcodeUnit, CodeInput in) throws EOFException {
      ...
      int a = byte1(opcodeUnit);
      int index = in.read();
      mod.agus.jcoderz.dx.io.IndexType indexType =
          mod.agus.jcoderz.dx.io.OpcodeInfo.getIndexType(opcode);
  ...
  FORMAT_22CS() {
    ...
    public DecodedInstruction decode(int opcodeUnit, CodeInput in) throws EOFException {
      ...
      int index = in.read();
      return new TwoRegisterDecodedInstruction(
          this, opcode, index, mod.agus.jcoderz.dx.io.IndexType.FIELD_OFFSET, 0, 0L, a, b);
    }
    ...
    public DecodedInstruction decode(int opcodeUnit, CodeInput in) throws EOFException {
      ...
      if (registerCount < 1 || registerCount > 5) {
        throw new DexException(
            "bogus registerCount: " + mod.agus.jcoderz.dx.util.Hex.uNibble(registerCount));
      }
  ...
  FORMAT_FILL_ARRAY_DATA_PAYLOAD() {
    ...
    public DecodedInstruction decode(int opcodeUnit, CodeInput in) throws EOFException {
      ...
      throw new DexException(
          "bogus element_width: " + mod.agus.jcoderz.dx.util.Hex.u2(elementWidth));
    }
  ...
  private static DecodedInstruction decodeRegisterList(
    ...
    return switch (registerCount) {
      case 0 -> new ZeroRegisterDecodedInstruction(format, opcode, index, indexType, 0, 0L);
      case 1 -> new OneRegisterDecodedInstruction(format, opcode, index, indexType, 0, 0L, a);
      case 2 -> new TwoRegisterDecodedInstruction(format, opcode, index, indexType, 0, 0L, a, b);
      case 3 ->
          new ThreeRegisterDecodedInstruction(format, opcode, index, indexType, 0, 0L, a, b, c);
      case 4 ->
          new FourRegisterDecodedInstruction(format, opcode, index, indexType, 0, 0L, a, b, c, d);
      case 5 ->
          new FiveRegisterDecodedInstruction(
              format, opcode, index, indexType, 0, 0L, a, b, c, d, e);
      default -> throw new DexException("bogus registerCount: " + Hex.uNibble(registerCount));
    };
  }
