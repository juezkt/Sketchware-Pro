...
public final class DexTranslationAdvice implements TranslationAdvice {
  ...
  public boolean hasConstantOperation(
      mod.agus.jcoderz.dx.rop.code.Rop opcode, RegisterSpec sourceA, RegisterSpec sourceB) {
    ...
    if (!(sourceB.getTypeBearer() instanceof mod.agus.jcoderz.dx.rop.cst.CstInteger)) {
      ...
      if (sourceA.getTypeBearer() instanceof mod.agus.jcoderz.dx.rop.cst.CstInteger
        ...
        mod.agus.jcoderz.dx.rop.cst.CstInteger cst =
            (mod.agus.jcoderz.dx.rop.cst.CstInteger) sourceA.getTypeBearer();
    ...
    mod.agus.jcoderz.dx.rop.cst.CstInteger cst =
        (mod.agus.jcoderz.dx.rop.cst.CstInteger) sourceB.getTypeBearer();

    return switch (opcode.getOpcode()) {
      // These have 8 and 16 bit cst representations
      case RegOps.REM, RegOps.ADD, RegOps.MUL, RegOps.DIV, RegOps.AND, RegOps.OR, RegOps.XOR ->
          cst.fitsIn16Bits();
  ...
  /** {@inheritDoc} */
  @Override
  public boolean requiresSourcesInOrder(
      Rop opcode, mod.agus.jcoderz.dx.rop.code.RegisterSpecList sources) {
