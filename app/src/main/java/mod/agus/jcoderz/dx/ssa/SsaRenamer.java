...
public class SsaRenamer implements Runnable {
  ...
  public SsaRenamer(mod.agus.jcoderz.dx.ssa.SsaMethod ssaMeth) {
    ...
    mod.agus.jcoderz.dx.rop.code.RegisterSpec[] initialRegMapping =
        new mod.agus.jcoderz.dx.rop.code.RegisterSpec[ropRegCount];
    for (int i = 0; i < ropRegCount; i++) {
      // everyone starts with a version 0 register
      initialRegMapping[i] =
          mod.agus.jcoderz.dx.rop.code.RegisterSpec.make(i, mod.agus.jcoderz.dx.rop.type.Type.VOID);
   ...
   */
  private static mod.agus.jcoderz.dx.rop.code.RegisterSpec[] dupArray(
      mod.agus.jcoderz.dx.rop.code.RegisterSpec[] orig) {
    mod.agus.jcoderz.dx.rop.code.RegisterSpec[] copy =
        new mod.agus.jcoderz.dx.rop.code.RegisterSpec[orig.length];
  ...
  private class BlockRenamer implements mod.agus.jcoderz.dx.ssa.SsaInsn.Visitor {
    ...
    private final HashMap<mod.agus.jcoderz.dx.ssa.SsaInsn, mod.agus.jcoderz.dx.ssa.SsaInsn>
        insnsToReplace;
    ...
    BlockRenamer(final mod.agus.jcoderz.dx.ssa.SsaBasicBlock block) {
      ...
      insnsToReplace =
          new HashMap<mod.agus.jcoderz.dx.ssa.SsaInsn, mod.agus.jcoderz.dx.ssa.SsaInsn>();
    ...
    public void process() {
      ...
      for (mod.agus.jcoderz.dx.ssa.SsaBasicBlock child : block.getDomChildren()) {
        if (child != block) {
          ...
          mod.agus.jcoderz.dx.rop.code.RegisterSpec[] childStart =
              first ? currentMapping : dupArray(currentMapping);
    ...
    public void visitMoveInsn(NormalSsaInsn insn) {
      ...
      if (!Optimizer.getPreserveLocals()
        ...
        mod.agus.jcoderz.dx.rop.code.RegisterSpecList ssaSources =
            RegisterSpecList.make(
                mod.agus.jcoderz.dx.rop.code.RegisterSpec.make(
                    ssaReg.getReg(), ssaReg.getType(), newLocal));
    ...
    private void updateSuccessorPhis() {
      mod.agus.jcoderz.dx.ssa.PhiInsn.Visitor visitor =
          new mod.agus.jcoderz.dx.ssa.PhiInsn.Visitor() {
            @Override
