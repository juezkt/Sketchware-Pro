...
public class DirectClassFile implements ClassFile {
   ...
   */
  public DirectClassFile(
      mod.agus.jcoderz.dx.util.ByteArray bytes, String filePath, boolean strictParse) {
    if (bytes == null) {
  ...
  public mod.agus.jcoderz.dx.cf.code.BootstrapMethodsList getBootstrapMethods() {
    mod.agus.jcoderz.dx.cf.attrib.AttBootstrapMethods bootstrapMethodsAttribute =
        (mod.agus.jcoderz.dx.cf.attrib.AttBootstrapMethods)
            getAttributes().findFirst(AttBootstrapMethods.ATTRIBUTE_NAME);
  ...
  /**
   * Constructs and returns an instance of {@link mod.agus.jcoderz.dx.rop.type.TypeList} whose data
   * comes from the bytes of this instance, interpreted as a list of constant pool indices for
   * classes, which are in turn translated to type constants. Instance construction will fail if any
   * of the (alleged) indices turn out not to refer to constant pool entries of type {@code Class}.
   *
  ...
  private void parse() {
    try {
      ...
      mod.agus.jcoderz.dx.cf.iface.ParseException pe =
          new mod.agus.jcoderz.dx.cf.iface.ParseException(ex);
  ...
  private void parse0() {
    ...
    if (observer != null) {
      ...
      observer.parsed(
          bytes, 4, 2, "minor_version: " + mod.agus.jcoderz.dx.util.Hex.u2(getMinorVersion0()));
      observer.parsed(
          bytes, 6, 2, "major_version: " + mod.agus.jcoderz.dx.util.Hex.u2(getMajorVersion0()));
    }
    ...
    if (strictParse) {
      ...
      if (!isGoodMagic(getMagic0())) {
        throw new mod.agus.jcoderz.dx.cf.iface.ParseException(
            "bad class file magic (" + mod.agus.jcoderz.dx.util.Hex.u4(getMagic0()) + ")");
      }
    ...
    if (observer != null) {
      ...
      observer.parsed(
          bytes, at + 6, 2, "interfaces_count: " + mod.agus.jcoderz.dx.util.Hex.u2(count));
      if (count != 0) {
    ...
    mod.agus.jcoderz.dx.cf.direct.AttributeListParser alParser =
        new mod.agus.jcoderz.dx.cf.direct.AttributeListParser(
            this, AttributeFactory.CTX_CLASS, at, attributeFactory);
  ...
  /**
   * Implementation of {@link mod.agus.jcoderz.dx.rop.type.TypeList} whose data comes directly from
   * the bytes of an instance of this (outer) class, interpreted as a list of constant pool indices
   * for classes which are in turn returned as type constants. Instance construction will fail if
   * any of the (alleged) indices turn out not to refer to constant pool entries of type {@code
   * Class}.
   */
