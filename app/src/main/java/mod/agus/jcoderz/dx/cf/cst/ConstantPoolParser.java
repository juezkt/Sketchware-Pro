...
public final class ConstantPoolParser {
  ...
  private void parse() {
    ...
    if (observer != null) {
      observer.parsed(
          bytes, 8, 2, "constant_pool_count: " + mod.agus.jcoderz.dx.util.Hex.u2(offsets.length));
  ...
  private void determineOffsets() {
    ...
    for (int i = 1; i < offsets.length; i += lastCategory) {
      ...
      try {
        ...
        ex.addContext(
            "...while preparsing cst "
                + mod.agus.jcoderz.dx.util.Hex.u2(i)
                + " at offset "
                + mod.agus.jcoderz.dx.util.Hex.u4(at));
        throw ex;
      }
    }
  ...
  private mod.agus.jcoderz.dx.rop.cst.Constant parse0(int idx, BitSet wasUtf8) {
    ...
    try {
        ...
        case CONSTANT_Class:
          {
            ...
            mod.agus.jcoderz.dx.rop.cst.CstString name =
                (mod.agus.jcoderz.dx.rop.cst.CstString) parse0(nameIndex, wasUtf8);
        ...
        case CONSTANT_Fieldref:
          {
            ...
            mod.agus.jcoderz.dx.rop.cst.CstType type =
                (mod.agus.jcoderz.dx.rop.cst.CstType) parse0(classIndex, wasUtf8);
            int natIndex = bytes.getUnsignedShort(at + 3);
            mod.agus.jcoderz.dx.rop.cst.CstNat nat =
                (mod.agus.jcoderz.dx.rop.cst.CstNat) parse0(natIndex, wasUtf8);
        ...
        case CONSTANT_NameAndType:
          {
            ...
            mod.agus.jcoderz.dx.rop.cst.CstString descriptor =
                (mod.agus.jcoderz.dx.rop.cst.CstString) parse0(descriptorIndex, wasUtf8);
      ...
      ex.addContext(
          "...while parsing cst "
              + mod.agus.jcoderz.dx.util.Hex.u2(idx)
              + " at offset "
              + mod.agus.jcoderz.dx.util.Hex.u4(at));
      throw ex;
  ...
  private static int getMethodHandleTypeForKind(int kind) {
    return switch (kind) {
      ...
      case MethodHandleKind.REF_invokeVirtual -> CstMethodHandle.METHOD_HANDLE_TYPE_INVOKE_INSTANCE;
      case MethodHandleKind.REF_invokeStatic -> CstMethodHandle.METHOD_HANDLE_TYPE_INVOKE_STATIC;
      case MethodHandleKind.REF_invokeSpecial -> CstMethodHandle.METHOD_HANDLE_TYPE_INVOKE_DIRECT;
