...
public class ConstCollector {
  ...
  private void run() {
    ...
    ArrayList<mod.agus.jcoderz.dx.rop.cst.TypedConstant> constantList = getConstsSortedByCountUse();
    ...
    HashMap<mod.agus.jcoderz.dx.rop.cst.TypedConstant, mod.agus.jcoderz.dx.rop.code.RegisterSpec>
        newRegs =
            new HashMap<
                mod.agus.jcoderz.dx.rop.cst.TypedConstant,
                mod.agus.jcoderz.dx.rop.code.RegisterSpec>(toCollect);
    ...
    for (int i = 0; i < toCollect; i++) {
      ...
      if (constRop.getBranchingness() == Rop.BRANCH_NONE) {
        start.addInsnToHead(
            new PlainCstInsn(
                mod.agus.jcoderz.dx.rop.code.Rops.opConst(cst),
        ...
        mod.agus.jcoderz.dx.ssa.SsaBasicBlock successorBlock = entryBlock.getPrimarySuccessor();
  ...
  private ArrayList<mod.agus.jcoderz.dx.rop.cst.TypedConstant> getConstsSortedByCountUse() {
    ...
    for (int i = 0; i < regSz; i++) {
      ...
      mod.agus.jcoderz.dx.rop.cst.TypedConstant cst =
          (mod.agus.jcoderz.dx.rop.cst.TypedConstant) typeBearer;
    ...
    ArrayList<mod.agus.jcoderz.dx.rop.cst.TypedConstant> constantList =
        new ArrayList<mod.agus.jcoderz.dx.rop.cst.TypedConstant>();
    for (Map.Entry<mod.agus.jcoderz.dx.rop.cst.TypedConstant, Integer> entry :
        countUses.entrySet()) {
      if (entry.getValue() > 1) {
    ...
    Collections.sort(
        constantList,
        new Comparator<mod.agus.jcoderz.dx.rop.cst.Constant>() {
          @Override
   ...
   */
  private void fixLocalAssignment(
      mod.agus.jcoderz.dx.rop.code.RegisterSpec origReg,
