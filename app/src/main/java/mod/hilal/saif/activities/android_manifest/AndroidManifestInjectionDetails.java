...
public class AndroidManifestInjectionDetails extends BaseAppCompatActivity {
  ...
  private void setToolbar() {
    ...
    if (!str.equals("Attributes for all activities")
        && !str.equals("Application Attributes")
        && !str.equals("Application Permissions")) {
  ...
  private void showDial(int pos) {
    ...
    DialogCreateNewFileLayoutBinding attributeBinding =
  ...
  private void showAddDial() {
    ...
    dialog.setPositiveButton(
        ...
        (dialog1, which) -> {
          String fstr =
              attributeBinding.inputRes.getText().toString().trim()
                  + ":"
                  + attributeBinding.inputAttr.getText().toString().trim()
                  + "=\""
                  + attributeBinding.inputValue.getText().toString().trim()
                  + "\"";
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
      ...
      try {
        ...
        SpannableString spannableString =
            new SpannableString((String) _data.get(position).get("value"));
        spannableString.setSpan(
            new ForegroundColorSpan(violet),
            0,
            ((String) _data.get(position).get("value")).indexOf(":"),
            33);
        spannableString.setSpan(
            new ForegroundColorSpan(onSurface),
            ((String) _data.get(position).get("value")).indexOf(":"),
            ((String) _data.get(position).get("value")).indexOf("=") + 1,
            33);
        spannableString.setSpan(
            new ForegroundColorSpan(green),
            ((String) _data.get(position).get("value")).indexOf("\""),
            ((String) _data.get(position).get("value")).length(),
            33);
