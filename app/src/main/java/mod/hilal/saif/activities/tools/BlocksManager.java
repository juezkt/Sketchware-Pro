...
public class BlocksManager extends BaseAppCompatActivity {
        ...
        new ItemTouchHelper(
            new ItemTouchHelper.SimpleCallback(ItemTouchHelper.UP | ItemTouchHelper.DOWN, 0) {
              ...
              public boolean onMove(
                ...
                Objects.requireNonNull(binding.paletteRecycler.getAdapter())
              ...
              public void onChildDraw(
                ...
                if (actionState == ItemTouchHelper.ACTION_STATE_DRAG) {
                  binding.background.setClipChildren(
                      !isItNearTrash(draggedView, binding.recycleBin));
                  if (isItInTrash(draggedView, binding.recycleBin)) {
                    int pos = viewHolder.getBindingAdapterPosition();
                    binding.recycleBinCard.setAlpha(0.5f);
                    if (!isCurrentlyActive
                        && pos != RecyclerView.NO_POSITION
                        && pos < pallet_listmap.size()
                        && !isDialogShowing) {
  ...
  private void showBlockConfigurationDialog() {
    ...
    DialogBlockConfigurationBinding dialogBinding =
        DialogBlockConfigurationBinding.inflate(getLayoutInflater());
    ...
    dialog.b(
        ...
        view -> {
          ConfigActivity.setSetting(
              ...
              Objects.requireNonNull(dialogBinding.palettesPath.getText()).toString());
          ConfigActivity.setSetting(
              ConfigActivity.SETTING_BLOCKMANAGER_DIRECTORY_BLOCK_FILE_PATH,
              Objects.requireNonNull(dialogBinding.blocksPath.getText()).toString());
    ...
    dialog.configureDefaultButton(
        ...
        view -> {
          ConfigActivity.setSetting(
              ConfigActivity.SETTING_BLOCKMANAGER_DIRECTORY_PALETTE_FILE_PATH,
              ConfigActivity.getDefaultValue(
                  ConfigActivity.SETTING_BLOCKMANAGER_DIRECTORY_PALETTE_FILE_PATH));
          ConfigActivity.setSetting(
              ConfigActivity.SETTING_BLOCKMANAGER_DIRECTORY_BLOCK_FILE_PATH,
              ConfigActivity.getDefaultValue(
  ...
  private void showMoveToBinDialog(int position) {
    ...
    dialog.b(
        xB.b().a(activity, R.string.common_word_yes),
        v -> {
  ...
  private void readSettings() {
    ...
    if (FileUtil.isExistFile(blocks_dir) && isValidJson(FileUtil.readFile(blocks_dir))) {
      ...
      SketchwareUtil.showFailedToParseJsonDialog(
          this, new File(blocks_dir), "Custom Blocks", v -> readSettings());
    }
  }
  ...
  private void refresh_list() {
    parsePaletteJson:
    {
      ...
      if (FileUtil.isExistFile(pallet_dir)
          && !(paletteJsonContent = FileUtil.readFile(pallet_dir)).isEmpty()) {
        try {
        ...
        SketchwareUtil.showFailedToParseJsonDialog(
            this, new File(pallet_dir), "Custom Block Palettes", v -> refresh_list());
      }
  ...
  private double getN(final double _p) {
    ...
    for (int i = 0; i < all_blocks_list.size(); i++) {
      if (Objects.requireNonNull(all_blocks_list.get(i).get("palette"))
          .toString()
          .equals(String.valueOf((long) (_p)))) {
        n++;
      }
    }
    return (n);
  ...
  private void removeRelatedBlocks(final double _p) {
    ...
    for (int i = 0; i < all_blocks_list.size(); i++) {
      if (!(Double.parseDouble(
              Objects.requireNonNull(all_blocks_list.get(i).get("palette")).toString())
          == _p)) {
        if (Double.parseDouble(
                Objects.requireNonNull(all_blocks_list.get(i).get("palette")).toString())
            > _p) {
          ...
          m.put(
              ...
              String.valueOf(
                  (long)
                      (Double.parseDouble(
                              Objects.requireNonNull(all_blocks_list.get(i).get("palette"))
                                  .toString())
                          - 1)));
          newBlocks.add(m);
        } else {
  ...
  private void showPaletteDialog(
    ...
    dialogBinding.openColorPalette.setOnClickListener(
        v1 -> {
    ...
    dialog.b(
        ...
        v -> {
          String nameInput =
              Objects.requireNonNull(dialogBinding.nameEditText.getText()).toString();
          String colorInput =
              Objects.requireNonNull(dialogBinding.colorEditText.getText()).toString();
          ...
          if (PropertiesUtil.isHexColor(colorInput)) {
            ...
            if (!isEditing) {
              ...
              if (insertAtPosition == null) {
                ...
                FileUtil.writeFile(pallet_dir, getGson().toJson(pallet_listmap));
                Objects.requireNonNull(binding.paletteRecycler.getAdapter())
                    .notifyItemInserted(pallet_listmap.size() - 1);
                readSettings();
              } else {
                ...
                readSettings();
                Objects.requireNonNull(binding.paletteRecycler.getAdapter())
                    .notifyItemInserted(insertAtPosition);
  ...
  private boolean isItInTrash(View draggedView, View trash) {
    ...
    return draggedY <= (trashLocation[1] + draggedView.getMeasuredHeight() / 2)
        && draggedY >= ((trashLocation[1] - draggedView.getMeasuredHeight() / 2));
  }
  ...
  private boolean isItNearTrash(View draggedView, View trash) {
    ...
    return draggedY <= (trashLocation[1] + (draggedView.getMeasuredHeight() * 2) / 2)
        && draggedY >= ((trashLocation[1] - (draggedView.getMeasuredHeight() * 2) / 2));
  }
  ...
  public class PaletteAdapter extends RecyclerView.Adapter<PaletteAdapter.ViewHolder> {
    ...
    public PaletteAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
      PalletCustomviewBinding itemBinding =
          PalletCustomviewBinding.inflate(LayoutInflater.from(parent.getContext()), parent, false);
    ...
    public void onBindViewHolder(@NonNull PaletteAdapter.ViewHolder holder, int position) {
      ...
      holder.itemBinding.title.setText(
      ...
      holder.itemBinding.backgroundCard.setOnLongClickListener(
          v -> {
                  ...
                  switch (Objects.requireNonNull(item.getTitle()).toString()) {
                    case edit:
                      showPaletteDialog(
                          ...
                          Objects.requireNonNull(pallet_listmap.get(pos).get("color")).toString(),
                          null);
                      break;
                    ...
                    case delete:
                      new MaterialAlertDialogBuilder(BlocksManager.this)
                          .setTitle(
                              Objects.requireNonNull(pallet_listmap.get(pos).get("name"))
                                  .toString())
      ...
      holder.itemBinding.dragHandler.setOnTouchListener(
          (v, event) -> {
      ...
      holder.itemBinding.backgroundCard.setOnClickListener(
          v -> {
            Intent intent = new Intent(getApplicationContext(), BlocksManagerDetailsActivity.class);
            intent.putExtra(
                "position", String.valueOf((long) (holder.getAbsoluteAdapterPosition() + 9)));
            intent.putExtra("dirB", blocks_dir);
            intent.putExtra("dirP", pallet_dir);
            startActivity(intent);
          });
