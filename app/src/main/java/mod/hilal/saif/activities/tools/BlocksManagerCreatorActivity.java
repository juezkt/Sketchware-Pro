...
public class BlocksManagerCreatorActivity extends BaseAppCompatActivity {
  ...
  /**
   * Current mode of this activity, "edit" if editing a block, "add" if creating a new block and
   * "insert" if inserting a block above another
   */
  private String mode = "";
  ...
  private void initialize() {
    ...
    binding.selectType.setOnClickListener(
        v -> {
          List<String> types =
              Arrays.asList("regular", "c", "e", "s", "b", "d", "v", "a", "f", "l", "p", "h");
          List<String> choices =
              Arrays.asList(
          ...
          new MaterialAlertDialogBuilder(this)
              ...
              .setSingleChoiceItems(
                  ...
                  types.indexOf(binding.type.getText().toString()),
    ...
    binding.spec.addTextChangedListener(
        new BaseTextWatcher() {
          @Override
          public void onTextChanged(CharSequence s, int start, int before, int count) {
    ...
    binding.spec.setOnTouchListener(
        (v, event) -> {
  ...
  private void getBlockList() {
        ...
        for (int i = 0, blocksListSize = blocksList.size(); i < blocksListSize; i++) {
          ...
          if (name instanceof String) {
            ...
            SketchwareUtil.toastError(
                "Custom Block #" + i + " in current palette has an invalid name");
          }
        }
        return;
      }
    ...
    SketchwareUtil.showFailedToParseJsonDialog(
        this, new File(path), "Custom Blocks", v -> getBlockList());
    ...
    try {
      var block =
          new Rs(
              this,
              -1,
              binding.spec.getText().toString(),
              blockType,
              binding.name.getText().toString());
      block.e =
          PropertiesUtil.isHexColor(color)
              ? PropertiesUtil.parseColor(color)
  ...
  private void insertBlockAt(int position) {
    ...
    tempMap.put("name", binding.name.getText().toString());
    if (binding.type.getText().toString().equals("regular")
        || binding.type.getText().toString().isEmpty()) {
      tempMap.put("type", " ");
    } else {
      tempMap.put("type", binding.type.getText().toString());
    }
