...
public class AndroidManifestInjector {
  ...
  private static ArrayList<HashMap<String, Object>> readAndroidManifestAttributeInjections(
      String sc_id) {
    ...
    parseAttributes:
    {
      ...
      try {
        ...
        if (injections.exists()) {
          attributes =
              getGson()
                  .fromJson(FileUtil.readFile(injections.getAbsolutePath()), Helper.TYPE_MAP_LIST);
      ...
      SketchwareUtil.toastError(
          "Failed to parse AndroidManifest attribute injections; Reason: " + errorMessage);
    }
    ...
    for (int i = 0; i < attributes.size(); i++) {
      ...
      if (name instanceof String) {
        ...
        if (value instanceof String) {
          ...
          SketchwareUtil.toastError(
              "Invalid AndroidManifest attribute injection value in attribute #" + (i + 1));
        }
      } else {
        SketchwareUtil.toastError(
            "Invalid AndroidManifest attribute injection name in attribute #" + (i + 1));
      }
    }
  }
  ...
  public static boolean getActivityAttrs(XmlBuilder nx, String projectId, String actName) {
    ArrayList<HashMap<String, Object>> attributes =
        readAndroidManifestAttributeInjections(projectId);
  ...
  public static boolean isActivityAttributeUsed(
      String attribute, String sc_id, String activityName) {
  ...
  public static String mHolder(String m, String projectId) {
                    ...
                    if (v.matches("^		<[a-zA-Z_-]+[^>]")) {
                      ...
                      if (hasShortClosing) {
                        manifestLines.set(
                            ...
                            manifestLines
                                .get(q - 1)
                                .replace(spaceBeforeClosing ? "\" />" : "\"/>", "\">"));
                        manifestLines.set(
                            q - 1,
                            manifestLines.get(q - 1) + "\r\n" + value + "\r\n" + "</activity>");
                      } else {
    ...
    if (appComponents.exists()) {
      ...
      if (!(appComponentsContent = FileUtil.readFile(appComponents.getAbsolutePath()))
          .trim()
          .isEmpty()) {
