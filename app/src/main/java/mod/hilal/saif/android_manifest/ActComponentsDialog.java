...
public class ActComponentsDialog extends Dialog {
  ...
  public ActComponentsDialog(Activity activity, String sc_id, String activityName) {
    super(activity);
    ACTIVITIES_COMPONENTS_FILE_PATH =
        FileUtil.getExternalStorageDir()
            + "/.sketchware/data/"
            + sc_id
            + "/Injection/androidmanifest/activities_components.json";
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    base.setBackground(
        new DialogButtonGradientDrawable().getIns((int) getDip(4), 0, Color.WHITE, Color.WHITE));
  ...
  private void addControl() {
    ...
    linearLayout.setLayoutParams(
        new LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0.0f));
    ...
    ((LinearLayout.LayoutParams) cancel.getLayoutParams())
        .setMargins((int) getDip(8), (int) getDip(8), (int) getDip(8), (int) getDip(8));
    ...
    if (codeEditor.dark_theme) {
      save.setBackground(
          new DialogButtonGradientDrawable().getIns((int) getDip(4), 0, 0xff333333, 0xff333333));
      cancel.setBackground(
          new DialogButtonGradientDrawable().getIns((int) getDip(4), 0, 0xff333333, 0xff333333));
    } else {
      save.setBackground(
          new DialogButtonGradientDrawable().getIns((int) getDip(4), 0, 0xff2196f3, 0xff2196f3));
      cancel.setBackground(
          new DialogButtonGradientDrawable().getIns((int) getDip(4), 0, 0xff2196f3, 0xff2196f3));
    }
            ...
            activity.runOnUiThread(
                () -> {
                  if (codeEditor.dark_theme) {
                    linearLayout.setBackgroundColor(0xff292929);
                    save.setBackground(
                        new DialogButtonGradientDrawable()
        ...
        v -> {
          ...
          if (FileUtil.isExistFile(ACTIVITIES_COMPONENTS_FILE_PATH)) {
            ArrayList<HashMap<String, Object>> activitiesComponents =
                getGson()
                    .fromJson(
                        FileUtil.readFile(ACTIVITIES_COMPONENTS_FILE_PATH), Helper.TYPE_MAP_LIST);
            for (int i = 0; i < activitiesComponents.size(); i++) {
              if (activitiesComponents.get(i).get("name").equals(activityName)) {
                ...
                FileUtil.writeFile(
                    ACTIVITIES_COMPONENTS_FILE_PATH, getGson().toJson(activitiesComponents));
            ...
            map.put("value", codeEditor.getText());
            arrayList.add(map);
            FileUtil.writeFile(ACTIVITIES_COMPONENTS_FILE_PATH, getGson().toJson(arrayList));
          }
          dismiss();
        });
