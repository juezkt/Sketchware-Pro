    ...
    try {
      ...
      for (int i = 0; i < arr.size(); i++) {
        if (n == 1) {
          if (!arr.get(i)
              .contains(
                  "//Ul5kmZqmO867OV0QTGOpjwX7MXmgzxzQBSZTf0Y16PnDXkhLsZfvF")) { // it doesn't
                                                                                // contain the
                                                                                // starting code.
            arr2.add(arr.get(i));
          }
        ...
        if (arr.get(i)
            .contains(
                "//Ul5kmZqmO867OV0QTGOpjwX7MXmgzxzQBSZTf0Y16PnDXkhLsZfvF")) { // if it contains the
                                                                              // starting code, then
                                                                              // stop injection.
          n = 0;
        }
        if (arr.get(i)
            .contains(
                "//3b5IqsVG57gNqLi7FBO2MeOW6iI7tOustUGwcA7HKXm0o7lovZ")) { // if it contains the
                                                                           // ending code, keep
                                                                           // going.
          n = 1;
        }
  ...
  public static String imports(String codes) {
    try {
      ...
      for (int i = 0; i < arr.size(); i++) {
        ...
        if (arr.get(i)
            .contains("//Ul5kmZqmO867OV0QTGOpjwX7MXmgzxzQBSZTf0Y16PnDXkhLsZfvF")) { // start
          n = 1;
        }
      }
      // assemble
    ...
    try {
      ...
      FileUtil.writeFile(
          ...
          FileUtil.getExternalStorageDir()
              .concat("/.sketchware/data/system/temp/")
              .concat(javaName));
