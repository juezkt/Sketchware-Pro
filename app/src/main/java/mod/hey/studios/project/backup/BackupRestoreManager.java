...
public class BackupRestoreManager {
  ...
  public static String getRestoreIntegratedLocalLibrariesMessage(
      boolean restoringMultipleBackups,
      int currentRestoringIndex,
      int totalAmountOfBackups,
      String filename) {
    if (!restoringMultipleBackups) {
      return "Looks like the backup file you selected contains some Local libraries. Do you want to"
                 + " copy them to your local_libs directory (if they do not already exist)?";
    } else {
      return "Looks like backup file "
          + filename
          + " ("
          + (currentRestoringIndex + 1)
          + " out of "
          + totalAmountOfBackups
          + ") contains some Local libraries. Do you want to copy them to your local_libs directory"
  ...
  public void backup(String sc_id, String project_name) {
    ...
    checkboxContainer.setLayoutParams(
    ...
    includeLocalLibraries.setLayoutParams(
        new LinearLayout.LayoutParams(
    ...
    includeUsedCustomBlocks.setLayoutParams(
        new LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT));
  ...
  public void restore() {
    ...
    fpd.setDialogSelectionListener(
        files -> {
  ...
  public void doRestore(String file, boolean restoreLocalLibs) {
    new RestoreAsyncTask(new WeakReference<>(act), file, restoreLocalLibs, projectsFragment)
        .execute("");
  }
  ...
  private static class BackupAsyncTask extends AsyncTask<String, Integer, String> {
    ...
    protected void onPreExecute() {
      ProgressMsgBoxBinding loadingDialogBinding =
          ProgressMsgBoxBinding.inflate(LayoutInflater.from(activityWeakReference.get()));
      loadingDialogBinding.tvProgress.setText("Creating backup...");
      dlg =
          new MaterialAlertDialogBuilder(activityWeakReference.get())
    ...
    protected void onPostExecute(String _result) {
      ...
      if (bm.getOutFile() != null) {
        SketchwareUtil.toast(
            "Successfully created backup to: " + bm.getOutFile().getAbsolutePath());
      } else {
  ...
  private static class RestoreAsyncTask extends AsyncTask<String, Integer, String> {
    ...
    protected void onPostExecute(String _result) {
      ...
      if (!bm.isRestoreSuccess() || error) {
        ...
        SketchwareUtil.toast(
            "Restored successfully. Refresh to see the project", Toast.LENGTH_LONG);
      }
    }
  }
}
