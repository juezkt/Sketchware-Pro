...
public class CustomBlocksDialog {
  ...
  public void show(Activity context, String sc_id) {
    this.sc_id = sc_id;
    ViewUsedCustomBlocksBinding dialogBinding =
        ViewUsedCustomBlocksBinding.inflate(context.getLayoutInflater());
    ...
    if (customBlocks.isEmpty()) {
      ...
      dialog.b(
          Helper.getResString(R.string.common_word_import),
          v -> {
    ...
    dialog.a(
        Helper.getResString(R.string.common_word_dismiss), Helper.getDialogDismissListener(dialog));
    dialog.show();
  }

  private void importAll(
      Context context, CustomBlocksManager customBlocksManager, ArrayList<BlockBean> list) {
    ArrayList<HashMap<String, Object>> blocksList = new ArrayList<>();
    String paletteDir =
        getConfigPath(ConfigActivity.SETTING_BLOCKMANAGER_DIRECTORY_PALETTE_FILE_PATH);
    ...
    new MaterialAlertDialogBuilder(context)
        ...
        .setSingleChoiceItems(
            ...
            selectedPalette.get(),
            (dialog, which) -> selectedPalette.set(which))
        .setNegativeButton(
            "Create new palette",
            (dialog, which) -> {
              showCreatePaletteDialog(
  ...
  private void showCreatePaletteDialog(
    ...
    DialogPaletteBinding binding =
        DialogPaletteBinding.inflate(((Activity) context).getLayoutInflater());

    binding.openColorPalette.setOnClickListener(
        v -> {
  ...
  public class BlocksAdapter extends RecyclerView.Adapter<BlocksAdapter.ViewHolder> {
    ...
    public BlocksAdapter(
        ArrayList<BlockBean> blockBeans, BiConsumer<Integer, Boolean> onCheckedChangeListener) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
      ItemCustomBlockBinding binding =
          ItemCustomBlockBinding.inflate(LayoutInflater.from(parent.getContext()), parent, false);
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
      ...
      private void setupCheckBox(@NonNull BlockBean block, int position) {
        ...
        if (canImport) {
          binding.checkBox.setChecked(
              Boolean.TRUE.equals(selectedBlocks.getOrDefault(position, false)));
      ...
      private boolean isBlockImportable(@NonNull BlockBean block) {
        ...
        return blockInfo.isMissing
            && !BlockLoader.getBlockFromProject(sc_id, block.opCode).isMissing;
      }
