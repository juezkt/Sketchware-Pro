...
public class SrcCodeEditor extends AppCompatActivity {
  public static final List<Pair<String, Class<? extends EditorColorScheme>>> KNOWN_COLOR_SCHEMES =
      List.of(
  ...
  public static void selectLanguage(CodeEditor ed, int which) {
    switch (which) {
      ...
      case 1:
        ed.setEditorLanguage(
            CodeEditorLanguages.loadTextMateLanguage(CodeEditorLanguages.SCOPE_NAME_KOTLIN));
        languageId = 1;
        break;
  ...
  private void initialize() {
    ...
    if (title.endsWith(".java")) {
      ...
      editor.setColorScheme(
          CodeEditorColorSchemes.loadTextMateColorScheme(CodeEditorColorSchemes.THEME_DRACULA));
      languageId = 1;
      ...
      if (ThemeUtils.isDarkThemeEnabled(getApplicationContext())) {
        ...
        editor.setColorScheme(
            CodeEditorColorSchemes.loadTextMateColorScheme(CodeEditorColorSchemes.THEME_GITHUB));
      }
      languageId = 2;
    }
  ...
  public void onBackPressed() {
    if (beforeContent.equals(editor.getText().toString())) {
    ...
    } else {
      {
        ...
        dialog.a(
            Helper.getResString(R.string.src_code_editor_unsaved_changes_dialog_warning_message));
  ...
  private void loadToolbar() {
    {
      ...
      toolbarMenu
          .add(Menu.NONE, Menu.NONE, Menu.NONE, "Word wrap")
          .setCheckable(true)
      ...
      toolbarMenu
          .add(Menu.NONE, Menu.NONE, Menu.NONE, "Auto complete")
          .setCheckable(true)
          .setChecked(local_pref.getBoolean("act_ac", true));
      toolbarMenu
          .add(Menu.NONE, Menu.NONE, Menu.NONE, "Auto complete symbol pair")
          .setCheckable(true)
  ...
  public static void showSwitchThemeDialog(
      Activity activity, CodeEditor codeEditor, DialogInterface.OnClickListener listener) {
    ...
    int selectedThemeIndex =
        knownColorSchemesProperlyOrdered.stream()
            .filter(pair -> pair.second.equals(currentScheme.getClass()))
            .map(KNOWN_COLOR_SCHEMES::indexOf)
            .findFirst()
            .orElse(-1);
    String[] themeItems =
        KNOWN_COLOR_SCHEMES.stream().map(pair -> pair.first).toArray(String[]::new);
  ...
  public static void showSwitchLanguageDialog(
      Activity activity, CodeEditor codeEditor, DialogInterface.OnClickListener listener) {
