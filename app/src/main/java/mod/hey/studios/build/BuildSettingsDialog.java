...
public class BuildSettingsDialog {
  ...
  public void show() {
    ...
    setRadioGroupOptions(
        getRadioGroup(VIEW_JAVA_VERSION),
        BuildSettingsDialogBridge.getAvailableJavaVersions(),
  ...
  private void setRadioGroupOptions(
      RadioGroup radioGroup, String[] options, String key, String defaultValue) {
          ...
          if (isChecked) {
            if (key.equals(SETTING_NO_HTTP_LEGACY)) {
              SketchwareUtil.toast(
                  "Note that this option may cause issues if RequestNetwork component is used");
            }
          }
        });
  }
