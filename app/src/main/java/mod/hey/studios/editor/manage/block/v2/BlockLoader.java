    ...
    if (customBlocksConfig.exists()) {
      try {
        ...
        SketchwareUtil.toastError(
            "Failed to get Custom Blocks for project " + sc_id + ": " + e.getMessage());
      }
    }
      ...
      if (map.containsKey("color")) {
        ...
        if (!map.containsKey("palette")) {
          ...
          if (mapPalette instanceof String) {
                  ...
                  if (mapPaletteNumber == indexInt) {
                    ...
                    if (paletteColor instanceof Integer) {
                      try {
                        ...
                        SketchwareUtil.toastError(
                            "Invalid color in Custom Block palette #" + (j + 1));
                      }
                    } else {
                      SketchwareUtil.toastError(
                          "Invalid color value type in Custom Block palette #" + (j + 1));
                    }
                  }
                } else {
                  SketchwareUtil.toastError(
                      "Invalid palette index value type in Custom Block palette #" + (j + 1));
                }
              }
            ...
            SketchwareUtil.toastError(
                "Invalid palette number value type in Custom Block #" + (i + 1));
            continue;
          }
        }
      }
