...
public class DexCompiler {
  ...
  public static void compileDexFiles(ProjectBuilder builder) throws CompilationFailedException {
    ...
    try {
      minApiLevel =
          Integer.parseInt(
              builder.settings.getValue(ProjectSettings.SETTING_MINIMUM_SDK_VERSION, "21"));
    } catch (NumberFormatException e) {
      throw new CompilationFailedException(
          "Invalid minSdkVersion specified in Project Settings" + e.getMessage());
    }

    if (!(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)) {
      throw new IllegalStateException(
          "Can't use d8 as API level " + Build.VERSION.SDK_INT + " < 26");
    }
    ...
    if (builder.proguard.isShrinkingEnabled()) {
      ...
      for (File file :
          FileUtil.listFilesRecursively(new File(builder.yq.compiledClassesPath), ".class")) {
