...
/** A class responsible for compiling a Project's resources. Supports AAPT2. */
public class ResourceCompiler {

  /**
   * About log tags: add ":" and the first letter of the function's name camelCase'd. For example,
   * in thisIsALongFunctionName, you should use this:
   *
   * <pre>
   *     TAG + ":tIALFN"
   * </pre>
  ...
  public ResourceCompiler(
      ...
      File aapt,
      boolean willBuildAppBundle,
      BuildProgressReceiver receiver) {
  ...
  public void compile() throws IOException, zy, MissingFileException {
    ...
    resourceCompiler.setProgressListener(
        new Compiler.ProgressListener() {
          @Override
  ...
  static class Aapt2Compiler implements Compiler {
    ...
    public Aapt2Compiler(ProjectBuilder buildHelper, File aapt2, boolean buildAppBundle) {
      ...
      compiledBuiltInLibraryResourcesDirectory =
          new File(SketchApplication.getContext().getCacheDir(), "compiledLibs");
    }
    ...
    public void compile() throws zy, MissingFileException {
      ...
      LogUtil.d(
          TAG + ":c",
          "Compiling built-in library resources took "
              + (System.currentTimeMillis() - savedTimeMillis)
              + " ms");
      savedTimeMillis = System.currentTimeMillis();
      compileLocalLibraryResources(outputPath);
      LogUtil.d(
          TAG + ":c",
          "Compiling local library resources took "
              + (System.currentTimeMillis() - savedTimeMillis)
              + " ms");
      savedTimeMillis = System.currentTimeMillis();
      ...
      LogUtil.d(
          TAG + ":c",
          "Compiling project generated resources took "
              + (System.currentTimeMillis() - savedTimeMillis)
              + " ms");
      savedTimeMillis = System.currentTimeMillis();
      ...
      LogUtil.d(
          TAG + ":c",
          "Compiling project imported resources took "
              + (System.currentTimeMillis() - savedTimeMillis)
              + " ms");
    ...
    public void link() throws zy, MissingFileException {
      ...
      args.add("-I");
      String customAndroidSdk =
          buildHelper.build_settings.getValue(BuildSettings.SETTING_ANDROID_JAR_PATH, "");
      ...
      for (String localLibraryAssetsDirectory :
          new ManageLocalLibrary(buildHelper.yq.sc_id).getAssets()) {
      ...
      for (Jp library : buildHelper.builtInLibraryManager.getLibraries()) {
        if (library.hasResources()) {
          args.add("-R");
          args.add(
              new File(compiledBuiltInLibraryResourcesDirectory, library.getName() + ".zip")
                  .getAbsolutePath());
        }
      }
        ...
        for (File file : filesInCompiledResourcesPath) {
          if (file.isFile()) {
            if (!file.getName().equals("project.zip")
                || !file.getName().equals("project-imported.zip")) {
              args.add("-R");
    ...
    private void compileLocalLibraryResources(String outputPath) throws zy, MissingFileException {
      ...
      LogUtil.d(
          ...
          "About to compile "
              + localLibrariesCount
              + " local "
              + (localLibrariesCount == 1 ? "library" : "libraries"));
    ...
    private void compileBuiltInLibraryResources() throws zy, MissingFileException {
      ...
      for (Jp builtInLibrary : buildHelper.builtInLibraryManager.getLibraries()) {
        if (builtInLibrary.hasResources()) {
          File cachedCompiledResources =
              new File(compiledBuiltInLibraryResourcesDirectory, builtInLibrary.getName() + ".zip");
          String libraryResources =
              BuiltInLibraries.getLibraryResourcesPath(builtInLibrary.getName());
          ...
          if (isBuiltInLibraryRecompilingNeeded(cachedCompiledResources)) {
            ...
            LogUtil.d(
                TAG + ":cBILR",
                "Skipped resource recompilation for built-in library " + builtInLibrary.getName());
          }
        }
      }
    }
      ...
      if (cachedCompiledResources.exists()) {
        try {
          ...
          return context
                  ...
                  .lastUpdateTime
              > cachedCompiledResources.lastModified();
        } catch (PackageManager.NameNotFoundException e) {
          LogUtil.e(
              TAG + ":iBILRN", "Couldn't get package info about ourselves: " + e.getMessage(), e);
        }
      } else {
        LogUtil.d(
            TAG + ":iBILRN",
            "File "
                + cachedCompiledResources.getAbsolutePath()
    ...
    private void compilingAssertDirectoryExists(String directoryPath) throws MissingFileException {
      ...
      if (!directory.exists()) {
        throw new MissingFileException(
            directory, MissingFileException.STEP_RESOURCE_COMPILING, true);
      }
    }
