...
public class BlockUtil {
  public static void loadMoreblockPreview(ViewGroup blockArea, String spec) {
    var moreblock =
        new Rs(
            blockArea.getContext(),
            0,
            ReturnMoreblockManager.getMbName(spec),
            ReturnMoreblockManager.getMoreblockType(spec),
            "definedFunc");
  ...
  /**
   * @param opCode Block op code like <code>"getArg"</code> (used in Events' heading/start Block) or
   *     <code>"getVar"</code> (type of Blocks in the Palette)
   * @return The Variable Block that's part of for example a MoreBlock or an Event, or <code>null
   *     </code> if its spec wasn't recognized.
   */
  @Nullable
