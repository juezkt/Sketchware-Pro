...
public class LibrarySettingsImporter {
  ...
  public LibrarySettingsImporter(
      String sc_id, Function<iC, ProjectLibraryBean> getLibrarySettings) {
    this.sc_id = sc_id;
  ...
  public void showDialog(Activity activity) {
    ...
    LinearLayout root =
        (LinearLayout) wB.a(activity, R.layout.manage_library_popup_project_selector);
    ...
    dialog.a(root);
    dialog.b(
        xB.b().a(activity, R.string.common_word_select),
        v -> {
          if (!mB.a()) {
            if (adapter.selectedProjectIndex >= 0) {
              var settings =
                  (ProjectLibraryBean) projects.get(adapter.selectedProjectIndex).get("settings");
  ...
  private class ProjectAdapter extends RecyclerView.Adapter<ProjectAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ProjectAdapter.ViewHolder viewHolder, int position) {
      ...
      viewHolder.projectVersion.setText(
          String.format(
              "%s(%s)", yB.c(projectMap, "sc_ver_name"), yB.c(projectMap, "sc_ver_code")));
    ...
    public ProjectAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
      return new ProjectAdapter.ViewHolder(
          LayoutInflater.from(parent.getContext())
              .inflate(R.layout.manage_library_popup_project_list_item, parent, false));
    }
