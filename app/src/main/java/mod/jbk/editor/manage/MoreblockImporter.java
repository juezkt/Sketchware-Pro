...
public class MoreblockImporter {
  ...
  public void importMoreblock(MoreBlockCollectionBean moreblock, Callback callback) {
    ...
    String blockName =
        ReturnMoreblockManager.getMbName(
            ReturnMoreblockManager.getMbNameWithTypeFromSpec(moreblock.spec));
  ...
  private void handleVariables(MoreBlockCollectionBean moreBlock) {
    ...
    if (toBeAddedVariables.isEmpty()
        && toBeAddedLists.isEmpty()
        && toBeAddedImages.isEmpty()
        && toBeAddedSounds.isEmpty()
        && toBeAddedFonts.isEmpty()) {
  ...
  private void showAutoAddDialog(MoreBlockCollectionBean moreBlock) {
    ...
    aBVar.b(
        xB.b().a(activity, R.string.common_word_continue),
        v -> {
    ...
    aBVar.a(
        xB.b().a(activity, R.string.common_word_cancel), Helper.getDialogDismissListener(aBVar));
    aBVar.show();
  }
  ...
  private void showEditMoreBlockNameDialog(MoreBlockCollectionBean moreBlock) {
    ...
    ((TextView) customView.findViewById(R.id.tv_favorites_guide))
        .setText(xB.b().a(activity, R.string.logic_more_block_desc_change_block_name));
    ...
    ZB validator =
        new ZB(
            activity,
            customView.findViewById(R.id.ti_input),
            uq.b,
            uq.a(),
            new ArrayList<>(moreBlockNamesWithoutReturnTypes));
    dialog.a(customView);
    dialog.b(
        xB.b().a(activity, R.string.common_word_save),
        v -> {
          if (validator.b()) {
            String moreBlockName =
                ReturnMoreblockManager.getMbName(
                    ReturnMoreblockManager.getMbNameWithTypeFromSpec(moreBlock.spec));
            moreBlock.spec =
                newName.getText().toString() + moreBlock.spec.substring(moreBlockName.length());
    ...
    dialog.a(
        xB.b().a(activity, R.string.common_word_cancel),
        v -> {
          mB.a(activity, newName);
          dialog.dismiss();
        });
      ...
      try {
        fileUtil.a(
            wq.a()
                ...
                + File.separator
                + image.resFullName,
            wq.g() + File.separator + sc_id + File.separator + image.resFullName);
      ...
      try {
        fileUtil.a(
            wq.a()
                ...
                + File.separator
                + a2.resFullName,
            wq.t() + File.separator + sc_id + File.separator + a2.resFullName);
        jC.d(sc_id).c.add(a2);
      } catch (Exception e) {
        e.printStackTrace();
      }
      ...
      try {
        fileUtil.a(
            wq.a()
                ...
                + File.separator
                + font.resFullName,
            wq.d() + File.separator + sc_id + File.separator + font.resFullName);
  ...
  private void maybeAddVariable(int variableType, String variableName) {
    ...
    for (Pair<Integer, String> toBeAddedVariable : toBeAddedVariables) {
      if (toBeAddedVariable.first == variableType
          && toBeAddedVariable.second.equals(variableName)) {
