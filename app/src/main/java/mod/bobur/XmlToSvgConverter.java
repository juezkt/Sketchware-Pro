...
public class XmlToSvgConverter {
    ...
    try {
      ...
      if (root.getTagName().equals("vector")) {
        ...
        svg.append("viewBox=\"0 0 ")
            .append(viewportWidth.isEmpty() ? "100" : viewportWidth)
            .append(" ")
            .append(viewportHeight.isEmpty() ? "100" : viewportHeight)
            .append("\" ");
      } else {
  ...
  private static void handlePath(Element path, StringWriter svg) {
    ...
    if (!fillColor.isEmpty()) {
      svg.append("fill=\"")
          .append(ColorEditorActivity.getColorValue(SketchApplication.getContext(), fillColor, 4))
          .append("\" ");
    ...
    if (!strokeColor.isEmpty()) {
      svg.append("stroke=\"")
          .append(ColorEditorActivity.getColorValue(SketchApplication.getContext(), strokeColor, 4))
          .append("\" ");
  ...
  public static ArrayList<String> getVectorDrawables(String sc_id) {
    ...
    FileUtil.listDir(
        "/storage/emulated/0/.sketchware/data/" + sc_id + "/files/resource/drawable/", cache);
  ...
  public static String getVectorFullPath(String sc_id, String fileName) {
    return "/storage/emulated/0/.sketchware/data/"
        + sc_id
        + "/files/resource/drawable/"
        + fileName
        + ".xml";
  }
  ...
  public static String getVectorColor(Element vectorElement) {
    ...
    // check colors file
    String filePath =
        "/storage/emulated/0/.sketchware/data/" + sc_id + "/files/resource/values/colors.xml";
    if (!FileUtil.isExistFile(filePath)) {
      filePath =
          "/storage/emulated/0/.sketchware/mysc/" + sc_id + "/app/src/main/res/values/colors.xml";
      if (!FileUtil.isExistFile(filePath)) {
        return "#FFFFFF";
      }
    }
