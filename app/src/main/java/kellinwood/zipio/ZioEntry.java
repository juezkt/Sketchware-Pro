...
public class ZioEntry implements Cloneable {
  ...
  public ZioEntry(String name, String sourceDataFile) throws IOException {
    ...
    if (log.isDebugEnabled())
      log.debug(String.format("Computing CRC for %s, size=%d", sourceDataFile, size));
  ...
  public void readLocalHeader() throws IOException {
    ...
    if (debug) log.debug(String.format("FILE POSITION: 0x%08x", input.getFilePointer()));
    ...
    if (signature != 0x04034b50) {
      throw new IllegalStateException(
          String.format(
              "Local header not found at pos=0x%08x, file=%s", input.getFilePointer(), filename));
    }
  ...
  public void writeLocalEntry(ZipOutput output) throws IOException {
    ...
    if (debug) {
      log.debug(String.format("Writing local header at 0x%08x - %s", localHeaderOffset, filename));
    }
    ...
    if (compression == 0) {
      ...
      long dataPos =
          output.getFilePointer()
              + // current position
              2
              + // plus size of extra data length
              filename.length()
              + // plus filename
    ...
    if (debug) log.debug(String.format("Data position 0x%08x", output.getFilePointer()));
    if (data != null) {
      output.writeBytes(data);
      if (debug) log.debug(String.format("Wrote %d bytes", data.length));
    } else {

      if (debug) log.debug(String.format("Seeking to position 0x%08x", dataPosition));
      ...
      while (totalCount != compressedSize) {
        int numRead =
            zipInput.in.read(buffer, 0, (int) Math.min(compressedSize - totalCount, bufferSize));
        if (numRead > 0) {
          output.writeBytes(buffer, 0, numRead);
          if (debug) log.debug(String.format("Wrote %d bytes", numRead));
          totalCount += numRead;
        } else
          throw new IllegalStateException(
              String.format(
  ...
  private void doRead(ZipInput input) throws IOException {
    ...
    // Bits 1, 2, 3, and 11 are allowed to be set (first bit is bit zero).  Any others are a
    // problem.
    if ((generalPurposeBits & 0xF7F1) != 0x0000) {
      throw new IllegalStateException(
          "Can't handle general purpose bits == " + String.format("0x%04x", generalPurposeBits));
    }
    ...
    generalPurposeBits =
        (short)
            (generalPurposeBits
                & 0x0800); // Don't write a data descriptor, preserve UTF-8 encoded filename bit
  ...
  public byte[] getData() throws IOException {
    ...
    while (count != size) {
      ...
      if (numRead < 0)
        throw new IllegalStateException(
            String.format("Read failed, expecting %d bytes, got %d instead", size, count));
      count += numRead;
    }
    return tmpdata;
  }
  ...
  public InputStream getInputStream(OutputStream monitorStream) throws IOException {
    ...
    if (entryOut != null) {
      ...
      if (compression == 0) return rawis;
        ...
        // Hacky, inflate using a sequence of input streams that returns 1 byte more than the actual
        // length of the data.
        // This extra dummy byte is required by InflaterInputStream when the data doesn't have the
        // header and crc fields (as it is in zip files).
        return new InflaterInputStream(
            new SequenceInputStream(rawis, new ByteArrayInputStream(new byte[1])),
            new Inflater(true));
      }
    }
