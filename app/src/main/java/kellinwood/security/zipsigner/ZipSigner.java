...
/**
 * This is a modified copy of com.android.signapk.SignApk.java. It provides an API to sign JAR files
 * (including APKs and Zip/OTA updates) in a way compatible with the mincrypt verifier, using SHA1
 * and RSA keys.
 *
 * <p>Please see the README.txt file in the root of this project for usage instructions.
 */
public class ZipSigner {
  ...
  public ZipSigner() throws ClassNotFoundException, IllegalAccessException, InstantiationException {
    // MD5 of the first 1458 bytes of the signature block generated by the key, mapped to the key
    // name
  ...
  public void issueLoadingCertAndKeysProgressEvent() {
    progressHelper.progress(
        ProgressEvent.PRORITY_IMPORTANT,
        resourceAdapter.getString(ResourceAdapter.Item.LOADING_CERTIFICATE_AND_KEY));
  }
    ...
    for (ZioEntry entry : byName.values()) {
      ...
      if (!entry.isDirectory()
        ...
        progressHelper.progress(
            ...
            resourceAdapter.getString(ResourceAdapter.Item.GENERATING_MANIFEST));
  ...
  private void generateSignatureFile(Manifest manifest, OutputStream out)
    ...
    for (Map.Entry<String, Attributes> entry : entries.entrySet()) {
      ...
      progressHelper.progress(
          ProgressEvent.PRORITY_NORMAL,
          resourceAdapter.getString(ResourceAdapter.Item.GENERATING_SIGNATURE_FILE));
    ...
    if (keySet.getSigBlockTemplate() != null) {
      ...
      try {
        ...
        Class generatorClass =
            Class.forName("kellinwood.security.zipsigner.optional.SignatureBlockGenerator");
        Method generatorMethod =
            generatorClass.getMethod("generate", KeySet.class, (new byte[1]).getClass());
  ...
  private void copyFiles(
    ...
    for (String name : names) {
      ...
      progressHelper.progress(
          ProgressEvent.PRORITY_NORMAL,
          resourceAdapter.getString(ResourceAdapter.Item.COPYING_ZIP_ENTRY, i, names.size()));
      i += 1;
  ...
  private void copyFiles(Map<String, ZioEntry> input, ZipOutput output) throws IOException {
    ...
    for (ZioEntry inEntry : input.values()) {
      ...
      progressHelper.progress(
          ProgressEvent.PRORITY_NORMAL,
          resourceAdapter.getString(ResourceAdapter.Item.COPYING_ZIP_ENTRY, i, input.size()));
      i += 1;
      output.write(inEntry);
    }
  }
  ...
  /** Sign the input with the default test key and certificate. Save result to output file. */
  ...
  public void signZip(String inputZipFilename, String outputZipFilename)
    ...
    if (inFile.equals(outFile)) {
      throw new IllegalArgumentException(
          resourceAdapter.getString(ResourceAdapter.Item.INPUT_SAME_AS_OUTPUT_ERROR));
    }

    progressHelper.initProgress();
    progressHelper.progress(
        ProgressEvent.PRORITY_IMPORTANT,
        resourceAdapter.getString(ResourceAdapter.Item.PARSING_CENTRAL_DIRECTORY));
  ...
  public void signZip(
      ...
      if (keyName == null)
        throw new AutoKeyException(
            resourceAdapter.getString(
                ResourceAdapter.Item.AUTO_KEY_SELECTION_ERROR,
                new File(outputZipFilename).getName()));
    ...
    try {
      ...
      progressHelper.progress(
          ProgressEvent.PRORITY_NORMAL,
          resourceAdapter.getString(ResourceAdapter.Item.GENERATING_SIGNATURE_BLOCK));
