...
public class SignatureBlockGenerator {
  ...
  /**
   * Sign the given content using the private and public keys from the keySet, and return the
   * encoded CMS (PKCS#7) data. Use of direct signature and DER encoding produces a block that is
   * verifiable by Android recovery programs.
   */
  public static byte[] generate(KeySet keySet, byte[] content) {
    try {
      ...
      JcaContentSignerBuilder jcaContentSignerBuilder =
          new JcaContentSignerBuilder(keySet.getSignatureAlgorithm()).setProvider("SC");
      ...
      SignerInfoGenerator signerInfoGenerator =
          jcaSignerInfoGeneratorBuilder.build(sha1Signer, keySet.getPublicKey());
