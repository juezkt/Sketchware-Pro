...
public class KeyStoreFileManager {
  ...
  static Logger logger = Logger.getLogger(KeyStoreFileManager.class);
  ...
  public static KeyStore loadKeyStore(String keystorePath, String encodedPassword)
    ...
    try {
      if (encodedPassword != null) {
        password =
            PasswordObfuscator.getInstance().decodeKeystorePassword(keystorePath, encodedPassword);
      }
  ...
  public static KeyStore loadKeyStore(String keystorePath, char[] password) throws Exception {
    ...
    try {
      ...
      // This type of exception is thrown when the keystore is a JKS keystore, but the file is
      // malformed
      // or the validity/password check failed.  In this case don't bother to attempt loading it as
      // a BKS keystore.
      throw x;
    } catch (Exception x) {
  ...
  public static void writeKeyStore(KeyStore ks, String keystorePath, char[] password)
    ...
    try {
      if (keystoreFile.exists()) {
        ...
        File tmpFile =
            File.createTempFile(keystoreFile.getName(), null, keystoreFile.getParentFile());
  ...
  public static String renameKey(
      String keystorePath, String storePass, String oldKeyName, String newKeyName, String keyPass)
      throws Exception {
    char[] keyPw = null;
    ...
    try {
      ...
      keyPw =
          PasswordObfuscator.getInstance().decodeAliasPassword(keystorePath, oldKeyName, keyPass);
  ...
  public static KeyStore.Entry getKeyEntry(
      String keystorePath, String storePass, String keyName, String keyPass) throws Exception {
    char[] keyPw = null;
   ...
   */
  public static void validateKeyPassword(
      String keystorePath, String keyName, String encodedPassword) throws Exception {
    char[] password = null;
    try {
      KeyStore ks = KeyStoreFileManager.loadKeyStore(keystorePath, (char[]) null);
      password =
          PasswordObfuscator.getInstance()
              .decodeAliasPassword(keystorePath, keyName, encodedPassword);
