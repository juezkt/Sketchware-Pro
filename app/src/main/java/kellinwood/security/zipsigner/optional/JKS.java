...
/**
 * This is an implementation of Sun's proprietary key store algorithm, called "JKS" for "Java Key
 * Store". This implementation was created entirely through reverse-engineering.
 *
 * <p>The format of JKS files is, from the start of the file:
 *
 * <ol>
 *   <li>Magic bytes. This is a four-byte integer, in big-endian byte order, equal to <code>
 *       0xFEEDFEED</code>.
 *   <li>The version number (probably), as a four-byte integer (all multibyte integral types are in
 *       big-endian byte order). The current version number (in modern distributions of the JDK) is
 *       2.
 *   <li>The number of entrires in this keystore, as a four-byte integer. Call this value <i>n</i>
 *   <li>Then, <i>n</i> times:
 *       <ol>
 *         <li>The entry type, a four-byte int. The value 1 denotes a private key entry, and 2
 *             denotes a trusted certificate.
 *         <li>The entry's alias, formatted as strings such as those written by <a
 *             href="http://java.sun.com/j2se/1.4.1/docs/api/java/io/DataOutput.html#writeUTF(java.lang.String)">DataOutput.writeUTF(String)</a>.
 *         <li>An eight-byte integer, representing the entry's creation date, in milliseconds since
 *             the epoch.
 *             <p>Then, if the entry is a private key entry:
 *             <ol>
 *               <li>The size of the encoded key as a four-byte int, then that number of bytes. The
 *                   encoded key is the DER encoded bytes of the <a
 *                   href="http://java.sun.com/j2se/1.4.1/docs/api/javax/crypto/EncryptedPrivateKeyInfo.html">EncryptedPrivateKeyInfo</a>
 *                   structure (the encryption algorithm is discussed later).
 *               <li>A four-byte integer, followed by that many encoded certificates, encoded as
 *                   described in the trusted certificates section.
 *             </ol>
 *             <p>Otherwise, the entry is a trusted certificate, which is encoded as the name of the
 *             encoding algorithm (e.g. X.509), encoded the same way as alias names. Then, a
 *             four-byte integer representing the size of the encoded certificate, then that many
 *             bytes representing the encoded certificate (e.g. the DER bytes in the case of X.509).
 *       </ol>
 *   <li>Then, the signature.
 * </ol>
 *
 * </ol>
 *
 * </ol>
 *
 * <p>(See <a href="genkey.java">this file</a> for some idea of how I was able to figure out these
 * algorithms)
 *
 * <p>Decrypting the key works as follows:
 *
 * <ol>
 *   <li>The key length is the length of the ciphertext minus 40. The encrypted key, <code>ekey
 *       </code>, is the middle bytes of the ciphertext.
 *   <li>Take the first 20 bytes of the encrypted key as a seed value, <code>K[0]</code>.
 *   <li>Compute <code>K[1] ... K[n]</code>, where <code>|K[i]| = 20</code>, <code>
 *       n = ceil(|ekey| / 20)</code>, and <code>K[i] = SHA-1(UTF-16BE(password) + K[i-1])</code>.
 *   <li><code>key = ekey ^ (K[1] + ... + K[n])</code>.
 *   <li>The last 20 bytes are the checksum, computed as <code>H =
 * SHA-1(UTF-16BE(password) + key)</code>. If this value does not match the last 20 bytes of the
 *       ciphertext, output <code>FAIL</code>. Otherwise, output <code>key</code>.
 * </ol>
 *
 * <p>The signature is defined as <code>SHA-1(UTF-16BE(password) +
 * US_ASCII("Mighty Aphrodite") + encoded_keystore)</code> (yup, Sun engineers are just that
 * clever).
 *
 * <p>(Above, SHA-1 denotes the secure hash algorithm, UTF-16BE the big-endian byte representation
 * of a UTF-16 string, and US_ASCII the ASCII byte representation of the string.)
 *
 ...
 *     <p>Changes by Ken Ellinwood: ** Fixed a NullPointerException in engineLoad(). This method
 *     must return gracefully if the keystore input stream is null. ** engineGetCertificateEntry()
 *     was updated to return the first cert in the chain for private key entries. ** Lowercase the
 *     alias names, otherwise keytool chokes on the file created by this code. ** Fixed the
 *     integrity check in engineLoad(), previously the exception was never thrown regardless of
 *     password value.
 */
public class JKS extends KeyStoreSpi {
  ...
  public Key engineGetKey(String alias, char[] password)
    ...
    } else return new SecretKeySpec(key, alias);
  }
  ...
  public void engineSetKeyEntry(String alias, Key key, char[] passwd, Certificate[] certChain)
    ...
    if (certChain != null) certChains.put(alias, certChain);
    else certChains.put(alias, new Certificate[0]);
  ...
  public String engineGetCertificateAlias(Certificate cert) {
    for (Iterator keys = trustedCerts.keySet().iterator(); keys.hasNext(); ) {
      ...
      if (cert.equals(trustedCerts.get(alias))) return alias;
    }
    return null;
  }
  ...
  public void engineLoad(InputStream in, char[] passwd)
    ...
    md.update("Mighty Aphrodite".getBytes(StandardCharsets.UTF_8)); // HAR HAR
    aliases.clear();
    trustedCerts.clear();
    privateKeys.clear();
    certChains.clear();
    ...
    if (din.readInt() != MAGIC) throw new IOException("not a JavaKeyStore");
    din.readInt(); // version no.
    final int n = din.readInt();
    aliases.ensureCapacity(n);
    if (n < 0) throw new LoadKeystoreException("Malformed key store");
    for (int i = 0; i < n; i++) {
