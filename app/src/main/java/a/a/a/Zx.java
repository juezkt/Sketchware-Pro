...
public class Zx extends PopupWindow {
  ...
  public Zx(
      Activity activity, int var3, boolean isTransparentColor, boolean isNoneColor, String scId) {
    super(activity);
    binding = ColorPickerBinding.inflate(activity.getLayoutInflater());
    sc_id = scId;
    initialize(activity, var3, isTransparentColor, isNoneColor);
  ...
  private void deleteAllSavedColors() {
    ...
    dialog.b(
        xB.b().a(activity, R.string.common_word_delete),
        v -> {
          colorPref.a();
    ...
    dialog.a(
        xB.b().a(activity, R.string.common_word_cancel), Helper.getDialogDismissListener(dialog));
    dialog.show();
  }
  ...
  public void initialize(
      Activity activity, int var3, boolean isTransparentColor, boolean isNoneColor) {
    ...
    binding.tiCustomColor.setHint(
        xB.b().a(activity, R.string.picker_color_hint_enter_hex_color_code));
    ...
    binding.tvAddColor.setOnClickListener(
        view -> {
          if (colorValidator.b()) {
            String formattedColor =
                String.format("#%8s", binding.etCustomColor.getText().toString())
                    .replaceAll(" ", "F");
    ...
    for (int j = 0; j < colorList.size(); ++j) {
      ...
      int finalJ = j;
      colorGroupItem.b.setOnClickListener(
          v -> {
            l = finalJ;
            if (finalJ == 0 && colorGroups.get(finalJ).length == 0) {
              bB.b(activity, xB.b().a(activity, R.string.picker_color_custom_color_not_found), 1)
                  .show();
            }
      ...
      colorGroupItem.b.setOnLongClickListener(
          v -> {
  ...
  private void initializeColorData(boolean isColorTransparent, boolean isColorNone) {
    ...
    if (sc_id != null)
      colorList.add(
          new ColorBean("#FFF6F6F6", "colors.xml", "#212121", R.drawable.checked_grey_32));
  ...
  private ColorBean[] geColorResBeans() {
    ...
    String clrsPath =
        FileUtil.getExternalStorageDir()
            ...
            .concat(sc_id.concat("/files/resource/values/colors.xml"));
    ...
    if (!color_res_list.isEmpty()) {
      ...
      while (index < color_res_list.size()) {
        try {
          int parsedColor =
              Color.parseColor((color_res_list.get(index).get("colorValue")).toString());
          int red = Color.red(parsedColor);
          int green = Color.green(parsedColor);
          int blue = Color.blue(parsedColor);
          ...
          colorBeans[index] =
              new ColorBean(
                  (color_res_list.get(index).get("colorValue")).toString(),
                  (color_res_list.get(index).get("colorName")).toString(),
                  count >= 2 ? "#212121" : "#ffffff",
                  R.drawable.checked_white_32);
        ...
        switch (eventType) {
          ...
          case XmlPullParser.END_TAG:
            if (tagName.equals("color")) {
              if (colorName != null
                  && isValidHexColor(
                      getColorValue(activity.getApplicationContext(), colorValue, 4))) {
  ...
  private class ColorsAdapter extends RecyclerView.Adapter<ColorsAdapter.ColorViewHolder> {
    ...
    public void onBindViewHolder(ColorViewHolder holder, int position) {
      ...
      holder.tvColorCode.setTextColor(
          ((ColorBean[]) colorGroups.get(l))[position].displayNameColor);
      holder.tvColorName.setTextColor(
          ((ColorBean[]) colorGroups.get(l))[position].displayNameColor);
      holder.layoutColorItem.setBackgroundColor(
          ((ColorBean[]) colorGroups.get(l))[position].colorCode);
      if (position == m && l == k) {
            ...
            v -> {
              if (colorPickerCallback != null) {
                if (tvColorCode.getText().toString().equals("TRANSPARENT")) {
                  ...
                  colorPickerCallback.a(
                      (String) tvColorName.getText(),
                      Color.parseColor(tvColorCode.getText().toString()));
                } else {
