...
public class BuiltInLibraryManager {
  ...
  /**
   * Add a built-in library to the project libraries list. Won't add a library if it's in the list
   * already, or it got excluded with {@link ExcludeBuiltInLibrariesActivity}.
   *
  ...
  public void addLibrary(String libraryName) {
    Optional<BuiltInLibraries.BuiltInLibrary> library =
        BuiltInLibraries.BuiltInLibrary.ofName(libraryName);
    ...
    if (!library.isPresent() || !excludedLibraries.contains(library.get())) {
      if (!libraryNames.contains(libraryName)) {
        Log.d(
            ProjectBuilder.TAG,
            "Added built-in library \"" + libraryName + "\" to project's dependencies");
        ...
        Log.v(
            ProjectBuilder.TAG,
            "Didn't add built-in library \"" + libraryName + "\" to project's dependencies again");
      }
    } else {
      Log.v(
          ProjectBuilder.TAG,
          "Didn't add built-in library \""
              + libraryName
              + "\" to project's dependencies as it's excluded");
