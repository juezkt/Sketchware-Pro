...
public class St extends qA {
  ...
  public final void updateImportButtonVisibility() {
    int selectedCount =
        (int) projectResourceBeans.stream().filter(resource -> resource.isSelected).count();

    if (selectedCount > 0) {
      actBinding.btnImport.setText(
          Helper.getResString(R.string.common_word_import_count, selectedCount));
  ...
  public void importSelectedFonts() {
    ...
    for (ProjectResourceBean resource : projectResourceBeans) {
      if (resource.isSelected) {
        ...
        selectedFonts.add(
            new ProjectResourceBean(
                ProjectResourceBean.PROJECT_RES_TYPE_FILE, resource.resName, path));
      }
    }

    if (!selectedFonts.isEmpty()) {
      ArrayList<ProjectResourceBean> fontCollection =
          ((ManageFontActivity) requireActivity()).projectFontsFragment.getProjectResourceBeans();
    ...
    public class fontHolder extends RecyclerView.ViewHolder {
      ...
      public fontHolder(ManageFontListItemBinding binding) {
        ...
        binding
            .getRoot()
            .setOnClickListener(
                view -> binding.chkSelect.setChecked(!binding.chkSelect.isChecked()));
