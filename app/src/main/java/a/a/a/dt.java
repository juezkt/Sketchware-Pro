...
public class dt extends LinearLayout {
  ...
  private void initialize(Activity activity) {
    ...
    binding.tvTitleBlockname.setText(
        Helper.getResString(R.string.logic_editor_more_block_title_name_of_block));
    binding.tvTitleAddVariable.setText(
        Helper.getResString(R.string.logic_editor_more_block_title_add_variable));
    blockNameValidator =
        new MoreblockValidator(activity, binding.tiName, uq.b, uq.a(), new ArrayList<>());
    labelTextValidator = new ZB(activity, binding.tiLabel, uq.b, uq.a(), new ArrayList<>());
    variableNameValidator =
        new ZB(activity, binding.tiVariableName, uq.b, uq.a(), new ArrayList<>());
    binding.tiName.setHint(
        Helper.getResString(R.string.logic_editor_more_block_hint_enter_new_block_name));
    binding.tiVariableName.setHint(
        Helper.getResString(R.string.logic_editor_more_block_hint_enter_variable_name));
    binding.tiLabel.setHint(
        Helper.getResString(R.string.logic_editor_more_block_hint_enter_block_label));
        ...
        view -> {
          if (!mB.a()) {
            if (variableNameValidator.b() && blockNameValidator.b()) {
              ...
              variablesSpecAndNamePair.add(
                  new Pair<>(fullSpec, binding.edVariableName.getText().toString()));
              updateBlockPreview(
                  binding.blockArea,
                  binding.removeArea,
                  block,
    ...
    binding.parameter.addTextChangedListener(
        new BaseTextWatcher() {
          @Override
    ...
    binding.add.setOnClickListener(
        v -> {
          if (!customVariableInvalid
              && !binding.name.getText().toString().isEmpty()
              && !binding.parameter.getText().toString().isEmpty()) {
            variablesSpecAndNamePair.add(
                new Pair<>(
                    binding.parameter.getText().toString(), binding.name.getText().toString()));
            updateBlockPreview(
                binding.blockArea,
                binding.removeArea,
                block,
  ...
  private void updateBlockPreview(
      ViewGroup blockArea,
      ViewGroup removeArea,
      Rs rs,
      String blockName,
      ArrayList<Pair<String, String>> variables) {
  ...
  public boolean b() {
    if (!binding.edName.getText().toString().isEmpty() && blockNameValidator.b()) {
      ...
      bB.b(
              getContext(),
              Helper.getResString(R.string.logic_editor_message_name_requied),
              Toast.LENGTH_SHORT)
          .show();
      return false;
    }
  ...
  public Pair<String, String> getBlockInformation() {
    ...
    return new Pair<>(
        ReturnMoreblockManager.injectMbType(var1, var1, getType()),
        ReturnMoreblockManager.injectMbType(block.T, var1, getType()));
  }
