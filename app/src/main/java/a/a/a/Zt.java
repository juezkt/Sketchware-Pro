...
public class Zt extends qA {
  ...
  public String getResourceFilePath(ProjectResourceBean resourceBean) {
    ...
    return dirPath + File.separator + resourceBean.resName + resFullName;
  }

  public String getFilePath(String resFullName) {
    return dirPath + File.separator + resFullName;
  }
  ...
  public void handleResourceImport(ArrayList<ProjectResourceBean> resourceBeans) {
    ...
    for (ProjectResourceBean resourceBean : resourceBeans) {
      ...
      if (isResourceNameExist(resourceName)) {
        ...
        ProjectResourceBean newResource =
            new ProjectResourceBean(
                ProjectResourceBean.PROJECT_RES_TYPE_FILE, resourceName, resourceBean.resFullName);
  ...
  public void setSelectingMode(boolean state) {
    ...
    actBinding
        .layoutBtnGroup
        .animate()
        .translationY(isSelecting ? 0F : 400F)
        .setDuration(200L)
        .start();
  ...
  public boolean isResourceNameExist(String resourceName) {
    return projectResourceBeans.stream().anyMatch(bean -> bean.resName.equals(resourceName));
  }
    ...
    for (int i = 0; i < projectResourceBeans.size(); i++) {
      ...
      if (resourceBean.isNew) {
        String fileExtension =
            resourceBean.resFullName.substring(resourceBean.resFullName.lastIndexOf("."));
        String newFileName = resourceBean.resName + fileExtension;
        resourceBean =
            new ProjectResourceBean(
                ProjectResourceBean.PROJECT_RES_TYPE_FILE, resourceBean.resName, newFileName);
  ...
  public View onCreateView(@NonNull LayoutInflater layoutInflater, ViewGroup root, Bundle bundle) {
    ...
    actBinding.btnDelete.setOnClickListener(
        view -> {
          if (isSelecting) {
    ...
    binding.fontList.addOnScrollListener(
    ...
    actBinding.fab.setOnClickListener(
        view -> {
          setSelectingMode(false);
          toFontActivity();
        });
  ...
  public class fontAdapter extends RecyclerView.Adapter<fontAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
      ManageFontListItemBinding binding =
          ManageFontListItemBinding.inflate(
        ...
        binding.layoutItem.setOnClickListener(
            view -> {
              ...
              if (isSelecting) {
                ...
                importFont(
                    projectResourceBeans.get(getLayoutPosition()).resFullName,
                    getResourceFilePath(projectResourceBeans.get(getLayoutPosition())));
              }
            });
