...
public class Yv extends qA {
  ...
  private void updateImportSoundsText() {
    int selectedSounds =
        (int) sounds.stream().filter(projectResourceBean -> projectResourceBean.isSelected).count();
    if (selectedSounds > 0) {
      actBinding.btnImport.setText(
          xB.b().a(getContext(), R.string.common_word_import_count, selectedSounds));
  ...
  public View onCreateView(
    ...
    binding.soundList.setLayoutManager(
    ...
    binding.tvGuide.setText(
        xB.b().a(requireActivity(), R.string.design_manager_sound_description_guide_add_sound));
    actBinding.btnImport.setOnClickListener(
        view -> {
          if (!mB.a()) {
            stopPlayback();
            ...
            if (!arrayList.isEmpty()) {
              ArrayList<ProjectResourceBean> d =
                  ((ManageSoundActivity) requireActivity()).projectSounds.sounds;
  ...
  private class Adapter extends SoundPlayingAdapter<Adapter.ViewHolder> {
    ...
    private class ViewHolder extends SoundPlayingAdapter.ViewHolder {
      ...
      public ViewHolder(@NonNull ManageSoundListItemBinding binding) {
        ...
        binding
            .getRoot()
            .setOnClickListener(v -> binding.chkSelect.setChecked(!binding.chkSelect.isChecked()));

        binding.chkSelect.setOnCheckedChangeListener(
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
      ...
      holder.binding.tvCurrenttime.setText(
          String.format(Locale.US, "%d:%02d", positionInS / 60, positionInS % 60));
      int durationInS = bean.totalSoundDuration / 1000;
      holder.binding.tvEndtime.setText(
          String.format(Locale.US, "%d:%02d", durationInS / 60, durationInS % 60));
      ...
      holder.binding.imgPlay.setImageResource(
          playing ? R.drawable.ic_mtrl_circle_pause : R.drawable.ic_mtrl_circle_play);
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
      ManageSoundListItemBinding binding =
          ManageSoundListItemBinding.inflate(inflater, parent, false);
  ...
  private void handleImportedSounds(ArrayList<ProjectResourceBean> resourceBeans) {
    ...
    for (ProjectResourceBean next : resourceBeans) {
      beans.add(
          new ProjectResourceBean(
              ProjectResourceBean.PROJECT_RES_TYPE_FILE, next.resName, next.resFullName));
    }
    if (!beans.isEmpty()) {
