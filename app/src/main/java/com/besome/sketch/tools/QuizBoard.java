...
public class QuizBoard extends LinearLayout implements View.OnClickListener {
  ...
  public void onClick(View var1) {
    if (!mB.a()) {
      a();
      int id = var1.getId();
      if (id == R.id.img_answer_o
          || id == R.id.img_answer_x
          || id == R.id.view_answer_a
          || id == R.id.view_answer_b) {
        f();
      }
      if (quizBean.type == 2) {
        var1.getId();
  ...
  public class a extends CountDownTimer {
    ...
    public void onTick(long millisUntilFinished) {
      (new Handler())
          .post(
              () ->
                  quizBinding.tvRemaingTime.setText(
                      String.valueOf(millisUntilFinished / 1000L + 1L)));
    }
  }
}
