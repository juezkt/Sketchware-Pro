...
public class PaletteSelector extends LinearLayout implements View.OnClickListener {
  ...
  private final int[] MainCategoriesIds = {0, 1, 2, 3, 4, 5, 6, 7, -1, 8};
  ...
  private void addSearchHeader() {
    ...
    searchTextView.setLayoutParams(
  ...
  private HashMap<String, Object> createPaletteMap(
      int index, String categoryName, int categoryColor) {
  ...
  private void showSearchDialog() {
    ...
    PalettesSearchDialogBinding binding =
        PalettesSearchDialogBinding.inflate(((Activity) context).getLayoutInflater());
    ...
    if (!searchValue.isEmpty()) {
      dialog.configureDefaultButton(
          context.getString(R.string.restore), v1 -> startSearch(dialog, ""));
    }
    dialog.a(binding.getRoot());
    binding.edittextSearchValue.setText(searchValue);
    binding.edittextSearchValue.addTextChangedListener(
        new SimpleTextWatcher(s -> validateSearch(s.toString(), binding.textInputLayoutSearch)));
    dialog.show();
  }
  ...
  public record SimpleTextWatcher(java.util.function.Consumer<CharSequence> onTextChanged)
