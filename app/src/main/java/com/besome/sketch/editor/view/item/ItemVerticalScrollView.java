...
public class ItemVerticalScrollView extends FrameLayout implements sy, ty {
  ...
  public void measureChild(View view, int parentWidthMeasureSpec, int parentHeightMeasureSpec) {
    ...
    parentWidthMeasureSpec =
        FrameLayout.getChildMeasureSpec(
            parentWidthMeasureSpec, getPaddingLeft() + getPaddingRight(), layoutParams.width);
    view.measure(
        parentWidthMeasureSpec,
  ...
  public void onDraw(Canvas canvas) {
    if (!isFixed) {
      ...
      canvas.drawLine(
          (float) scrollX, (float) scrollY, (float) measuredWidthX, (float) scrollY, paint);
      canvas.drawLine(
          (float) scrollX, (float) scrollY, (float) scrollX, (float) measuredHeightY, paint);
      canvas.drawLine(
          (float) measuredWidthX,
          (float) scrollY,
          (float) measuredWidthX,
          (float) measuredHeightY,
          paint);
      canvas.drawLine(
          (float) scrollX,
          (float) measuredHeightY,
    ...
    if (MeasureSpec.getMode(heightMeasureSpec) != MeasureSpec.UNSPECIFIED) {
      if (getChildCount() > 0) {
        ...
        if (child.getMeasuredHeight() < measuringSize) {
          child.measure(
              FrameLayout.getChildMeasureSpec(
                  widthMeasureSpec, heightMeasureSpec + getPaddingRight(), var4.width),
  ...
  public void setPadding(int left, int top, int right, int bottom) {
    super.setPadding(
        (int) wB.a(getContext(), (float) left),
