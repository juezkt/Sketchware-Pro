...
public class LogicTopMenu extends LinearLayout {
  ...
  public void setCopyActive(boolean active) {
    ...
    updateLayoutAppearance(
        binding.layoutCopy, active, R.color.scolor_green_normal, binding.tvCopy, binding.ivCopy);
  }

  public void setDeleteActive(boolean active) {
    isDeleteActive = active;
    updateLayoutAppearance(
        binding.layoutDelete, active, R.color.scolor_red_02, binding.tvDelete, binding.ivTrash);
  }
  ...
  private boolean isInsideArea(View layout, float x, float y) {
    ...
    return x > location[0]
        && x < (location[0] + layout.getWidth())
        && y > location[1]
        && y < (location[1] + layout.getHeight());
  }
}
