  ...
  private void a(ViewBean bean, String property) {
    ...
    switch (property) {
      ...
      case "property_margin" -> {
        ...
        a(
            property,
            layoutBean.marginLeft,
            layoutBean.marginTop,
            layoutBean.marginRight,
            layoutBean.marginBottom);
      }
      case "property_padding" -> {
        LayoutBean layoutBean = bean.layout;
        a(
            "property_padding",
            layoutBean.paddingLeft,
            layoutBean.paddingTop,
            layoutBean.paddingRight,
            layoutBean.paddingBottom);
      }
    ...
    for (int i = 0; i < childCount; ++i) {
      ...
      if (view instanceof PropertyInputItem inputItem) {
        switch (inputItem.getKey()) {
          ...
          case "property_weight" -> bean.layout.weight = Integer.parseInt(inputItem.getValue());
          case "property_weight_sum" ->
              bean.layout.weightSum = Integer.parseInt(inputItem.getValue());
          case "property_rotate" -> bean.image.rotate = Integer.parseInt(inputItem.getValue());
          ...
          case "property_progress" -> bean.progress = Integer.parseInt(inputItem.getValue());
        }
    ...
    if (!bean.id.equals(bean.preId)) {
      ...
      if (e.fileType == ProjectFileBean.PROJECT_FILE_TYPE_ACTIVITY) {
        ...
        for (Pair<String, String> moreblocks : jC.a(sc_id).i(e.getJavaName())) {
          String key = moreblocks.first + "_" + "moreBlock";
        ...
        for (String event : events) {
          ...
          if (blockBeans != null) {
            for (BlockBean blockBean : blockBeans) {
              ...
              if (classInfo != null
                  && classInfo.d()
                  && blockBean.spec.equals(
                      viewBinding
                          ? "binding." + ViewBindingBuilder.generateParameterFromId(bean.preId)
                          : bean.preId)) {
                blockBean.spec =
                    viewBinding
                        ? "binding." + ViewBindingBuilder.generateParameterFromId(bean.id)
                        : bean.id;
              } else {
                    ...
                    if (paramClassInfo.get(i).d()
                        && blockBean
                            ...
                            .equals(
                                viewBinding
                                    ? "binding."
                                        + ViewBindingBuilder.generateParameterFromId(bean.preId)
                                    : bean.preId)) {
                      blockBean.parameters.set(
                          i,
                          viewBinding
                              ? "binding." + ViewBindingBuilder.generateParameterFromId(bean.id)
                              : bean.id);
        ...
        for (ProjectFileBean projectFileBean : jC.b(sc_id).b()) {
          for (ViewBean viewBean : jC.a(sc_id).f(projectFileBean.getXmlName())) {
            if (viewBean.customView.equals(e.fileName)) {
              ...
              String key = viewBean.id + "_" + "onBindCustomView";
          ...
          while (true) {
            ...
            if (childCount < 0) {
              if (bean.customView.isEmpty() || bean.customView.equals("none")) {
                Iterator<Entry<String, ArrayList<BlockBean>>> blocks =
                    jC.a(sc_id).b(e.getJavaName()).entrySet().iterator();

                while (blocks.hasNext()) {
                  for (BlockBean blockBean : blocks.next().getValue()) {
                    if ("listSetCustomViewData".equals(blockBean.opCode)
                        && bean.id.equals(blockBean.parameters.get(0))) {
