...
public class AddSoundActivity extends BaseDialogActivity implements View.OnClickListener {
  ...
  private void pickSound() {
    ...
    startActivityForResult(
        Intent.createChooser(intent, getTranslatedString(R.string.common_word_choose)),
        REQUEST_CODE_SOUND_PICKER);
  }
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    nowPlayingProgressBar.setOnSeekBarChangeListener(
  ...
  private void saveSound() {
    if (isSoundValid(soundNameValidator)) {
      ...
      SketchwareUtil.copySafDocumentToTempFile(
          soundUri,
          this,
          FileUtil.getFileExtension(getFilenameOfPickedFile(soundUri)),
          tempFile -> {
            String soundFilePath = tempFile.getAbsolutePath();
            ProjectResourceBean projectResourceBean =
                new ProjectResourceBean(
                    ProjectResourceBean.PROJECT_RES_TYPE_FILE, soundName, soundFilePath);
            ...
            if (addToCollection.isChecked()) {
              try {
                ...
                if (e instanceof yy) {
                  ...
                  switch (message) {
                    case "duplicate_name":
                      bB.b(this, getTranslatedString(R.string.collection_duplicated_name), 1)
                          .show();
                      break;
                ...
                () -> {
                  if (nowPlayingPlayer == null) {
                    ...
                    nowPlayingProgress.setText(
                        String.format("%d:%02d", currentPosition / 60, currentPosition % 60));
  ...
  private void playSound(Uri uri) {
    ...
    try {
      ...
      nowPlayingPlayer.setAudioAttributes(
          new AudioAttributes.Builder()
              .setUsage(AudioAttributes.USAGE_MEDIA)
              .setContentType(AudioAttributes.CONTENT_TYPE_MUSIC)
              .build());
      nowPlayingPlayer.setOnPreparedListener(
          mp -> {
            playPause.setImageResource(R.drawable.ic_pause_circle_outline_black_36dp);
            ...
            nowPlayingFilename.setText(
                editingSound ? soundUri.getLastPathSegment() : getFilenameOfPickedFile(soundUri));

            mp.start();
            startNowPlayingProgressUpdater();
          });
      nowPlayingPlayer.setOnCompletionListener(
          mp -> {
            timer.cancel();
            playPause.setImageResource(R.drawable.ic_play_circle_outline_black_36dp);
            nowPlayingProgressBar.setProgress(0);
    ...
    try {
      if (editingSound) {
        ...
        mediaMetadataRetriever.setDataSource(
            getContentResolver().openFileDescriptor(soundUri, "r").getFileDescriptor());
      }
  ...
  private String getFilenameOfPickedFile(Uri input) {
    ...
    if (name.contains(".")) {
      ...
      Optional<String> optionalFilenameExtension =
          SketchwareUtil.doSingleStringContentQuery(input, MediaStore.MediaColumns.MIME_TYPE);
