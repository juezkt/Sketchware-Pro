...
public class AddImageActivity extends BaseDialogActivity implements View.OnClickListener {
  ...
  public void onCreate(Bundle bundle) {
    ...
    O.a(1);
    chk_collection.setText(
        xB.b().a(getApplicationContext(), R.string.design_manager_title_add_to_collection));
    tv_add_photo.setText(
        xB.b().a(getApplicationContext(), R.string.design_manager_image_title_add_image));
  ...
  private void refreshPreview() {
    preview.setImageBitmap(
        iB.a(
            iB.a(iB.a(imageFilePath, 1024, 1024), imageExifOrientation),
  ...
  private void pickImages(boolean allowMultiple) {
    try {
      ...
      startActivityForResult(
          Intent.createChooser(intent, xB.b().a(this, R.string.common_word_choose)), 215);
  ...
  private void setImageFromFile(String path) {
    ...
    if (ed_input_edittext != null
        && (ed_input_edittext.getText() == null || ed_input_edittext.getText().length() <= 0)) {
      ed_input_edittext.setText(
          path.substring(path.lastIndexOf("/") + 1, indexOfFilenameExtension));
    }
    try {
      ...
      try {
        ...
        if (!activity.multipleImagesPicked) {
          if (!activity.editing) {
            var image =
                new ProjectResourceBean(
                    ...
                    activity.ed_input_edittext.getText().toString().trim(),
                    activity.imageFilePath);
            image.savedPos = 1;
            image.isNew = true;
          ...
          while (i < activity.pickedImageUris.size()) {
            ...
            var image =
                new ProjectResourceBean(
                    ProjectResourceBean.PROJECT_RES_TYPE_FILE, imageName, imageFilePath);
            image.savedPos = 1;
            image.isNew = true;
