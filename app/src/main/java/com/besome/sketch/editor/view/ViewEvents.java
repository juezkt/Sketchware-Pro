  ...
  void setData(String sc_id, ProjectFileBean projectFileBean, ViewBean viewBean) {
    ...
    for (String event : viewEvents) {
      ...
      for (EventBean bean : alreadyAddedEvents) {
        if (bean.eventType == EventBean.EVENT_TYPE_VIEW
            && viewBean.id.equals(bean.targetId)
            && event.equals(bean.eventName)) {
      ...
      if (!event.equals("onBindCustomView")
          || (!viewBean.customView.isEmpty() && !viewBean.customView.equals("none"))) {
        EventBean eventBean =
            new EventBean(EventBean.EVENT_TYPE_VIEW, viewBean.type, viewBean.id, event);
        ...
        if (event.isSelected) {
          binding.imgIcon.setColorFilter(
              MaterialColors.getColor(
                  binding.tvTitle, com.google.android.material.R.attr.colorSecondary));
              ...
              v -> {
                ...
                dialog.b(
                    ...
                    view -> {
                      ...
                      bB.a(
                              getContext(),
                              xB.b().a(getContext(), R.string.common_message_complete_delete),
                              0)
                          .show();
          ...
          binding.imgIcon.setColorFilter(
              MaterialColors.getColor(
                  binding.tvTitle, com.google.android.material.R.attr.colorOutlineVariant));
        }
      }
    }
