...
public class AddEventActivity extends BaseAppCompatActivity implements View.OnClickListener {
  ...
  private void initialize() {
    ...
    for (var activityEvent : oq.a()) {
      ...
      if (!exists) {
        addableActivityEvents.add(
            new EventBean(EventBean.EVENT_TYPE_ACTIVITY, 0, activityEvent, activityEvent));
      }
    }
    ...
    if (views != null) {
      for (ViewBean view : views) {
        for (String viewEvent : oq.c(view.getClassInfo())) {
          boolean exists;
          if (viewEvent.equals("onBindCustomView")
              && (view.customView.isEmpty() || view.customView.equals("none"))) {
            exists = true;
          } else {
            exists = false;
            for (var existingEvent : jC.a(sc_id).g(projectFile.getJavaName())) {
          ...
          if (!exists) {
            addableViewEvents.add(
                new EventBean(EventBean.EVENT_TYPE_VIEW, view.type, view.id, viewEvent));
          }
        }
      }
    }
    if (components != null) {
      for (ComponentBean component : components) {
        for (String componentEvent : oq.a(component.getClassInfo())) {
          ...
          if (!exists) {
            addableComponentEvents.add(
                new EventBean(
                    EventBean.EVENT_TYPE_COMPONENT,
                    component.type,
    ...
    if (projectFile.hasActivityOption(ProjectFileBean.OPTION_ACTIVITY_FAB)
        && (fab = jC.a(sc_id).h(projectFile.getXmlName())) != null) {
      for (String fabEvent : oq.c(fab.getClassInfo())) {
        ...
        if (!exists) {
          addableViewEvents.add(
              new EventBean(EventBean.EVENT_TYPE_VIEW, fab.type, fab.id, fabEvent));
        }
      }
    }
      ...
      if (drawerViews != null) {
        for (ViewBean drawerView : drawerViews) {
          for (String drawerViewEvent : oq.c(drawerView.getClassInfo())) {
            ...
            if (!exists) {
              addableDrawerViewEvents.add(
                  new EventBean(
                      EventBean.EVENT_TYPE_DRAWER_VIEW,
                      drawerView.type,
                      drawerView.id,
                      drawerViewEvent));
            }
        ...
        if (!eventsToAdd.isEmpty() || !moreBlockView.a()) {
          if (!moreBlockView.a()) {
            if (!moreBlockView.b()) {
              ...
              jC.a(sc_id)
                  .a(projectFile.getJavaName(), blockInformation.first, blockInformation.second);
            }
          }
          if (!finished) {
            ...
            if (eventsToAdd.size() == 1) {
              bB.a(
                      ...
                      xB.b().a(getApplicationContext(), R.string.event_message_new_event),
                      bB.TOAST_NORMAL)
                  .show();
            } else if (eventsToAdd.size() > 1) {
              bB.a(
                      getApplicationContext(),
                      xB.b().a(getApplicationContext(), R.string.event_message_new_events),
                      bB.TOAST_NORMAL)
                  .show();
            }
            jC.a(sc_id).k();
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    events_preview.setLayoutManager(
        new LinearLayoutManager(getApplicationContext(), RecyclerView.HORIZONTAL, false));
  ...
  private class EventAdapter extends RecyclerView.Adapter<EventAdapter.ViewHolder> {
        ...
        itemView.setOnClickListener(
            v -> {
              if (!mB.a()) {
                ...
                EventBean event =
                    categories.get(categoryAdapter.lastSelectedCategory).get(lastSelectedEvent);
                if (event.isSelected) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
      ...
      holder.img_icon.setImageResource(
          EventBean.getEventIconResource(event.eventType, event.targetType));
      ...
      if (eventType == EventBean.EVENT_TYPE_ACTIVITY) {
        ...
        holder.tv_target_type.setText(
            ComponentBean.getComponentName(getApplicationContext(), event.targetType));
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
      return new ViewHolder(
          LayoutInflater.from(parent.getContext())
              .inflate(R.layout.fr_logic_list_item_addevent, parent, false));
    }
  ...
  private class CategoryAdapter extends RecyclerView.Adapter<CategoryAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
      View inflate =
          LayoutInflater.from(parent.getContext())
              .inflate(R.layout.fr_logic_list_category_icon_item, parent, false);
      inflate.setLayoutParams(
          new RecyclerView.LayoutParams(
              parent.getMeasuredWidth() / getItemCount(),
              (int) wB.a(getApplicationContext(), 44.0f)));
  ...
  private class EventsToAddAdapter extends RecyclerView.Adapter<EventsToAddAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
      return new ViewHolder(
          LayoutInflater.from(parent.getContext())
              .inflate(R.layout.fr_logic_list_preview_with_event_item, parent, false));
    }
