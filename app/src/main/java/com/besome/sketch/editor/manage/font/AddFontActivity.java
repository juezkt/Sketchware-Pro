...
public class AddFontActivity extends BaseDialogActivity implements View.OnClickListener {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    fontNameValidator =
        new WB2(this, binding.tiInput, uq.b, intent.getStringArrayListExtra("font_names"));
    if (intent.getIntExtra("request_code", -1) == 272) {
      ...
      binding.edInput.setText(
          ((ProjectResourceBean) intent.getParcelableExtra("resource_bean")).resName);
      binding.addToCollectionCheckbox.setEnabled(false);
    }
        ...
        view -> {
          if (!mB.a()) {
            ...
            startActivityForResult(
                Intent.createChooser(intent1, Helper.getResString(R.string.common_word_choose)),
                REQUEST_CODE_FONT_PICKER);
          }
        });
  }
    ...
    if (isFontValid(fontNameValidator)) {
      ...
      ProjectResourceBean resourceBean =
          new ProjectResourceBean(
              ProjectResourceBean.PROJECT_RES_TYPE_FILE, fontName, pickedFontFilePath);
          ...
          if (e instanceof yy) {
            switch (e.getMessage()) {
              case "duplicate_name" ->
                  bB.b(
                          this,
                          Helper.getResString(R.string.collection_duplicated_name),
                          Toast.LENGTH_LONG)
                      .show();
              case "file_no_exist" ->
                  bB.b(
                          this,
                          Helper.getResString(R.string.collection_no_exist_file),
                          Toast.LENGTH_LONG)
                      .show();
              case "fail_to_copy" ->
                  bB.b(
                          this,
                          Helper.getResString(R.string.collection_failed_to_copy),
                          Toast.LENGTH_LONG)
                      .show();
              default -> {}
            }
      ...
      bB.b(
              this,
              Helper.getResString(R.string.design_manager_message_no_font_selected),
              Toast.LENGTH_LONG)
          .show();
    }
  }
    ...
    if (requestCode == REQUEST_CODE_FONT_PICKER && resultCode == RESULT_OK) {
      ...
      String filenameExtension =
          FileUtil.getFileExtension(
              SketchwareUtil.getSafDocumentDisplayName(intentData).orElse(".ttf"));
          ...
          tempFontFile -> {
            ...
            try {
              ...
              String extractedFontName =
                  SketchwareUtil.getSafDocumentDisplayName(intentData)
                      .orElse("invalid.tff")
                      .toLowerCase();
