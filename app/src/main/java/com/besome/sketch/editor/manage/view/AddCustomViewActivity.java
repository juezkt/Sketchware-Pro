...
public class AddCustomViewActivity extends BaseDialogActivity implements View.OnClickListener {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    ((TextInputLayout) findViewById(R.id.ti_input))
        .setHint(Helper.getResString(R.string.design_manager_view_hint_enter_view_name));
    TextView description = findViewById(R.id.tv_desc);
    description.setText(
        Helper.getResString(R.string.design_manager_view_description_guide_use_custom_view));
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    if (requestCode == REQ_CD_PRESET_ACTIVITY && resultCode == RESULT_OK) {
      presetName =
          ((ProjectFileBean) Objects.requireNonNull(data.getParcelableExtra("preset_data")))
              .presetName;
    }
  }
  ...
  public void onClick(View v) {
    ...
    if (id == R.id.common_dialog_ok_button) {
      ...
      intent.putExtra(
          "project_file",
          new ProjectFileBean(
              ProjectFileBean.PROJECT_FILE_TYPE_CUSTOM_VIEW, customViewName.getText().toString()));
      if (presetName != null) {
      ...
      bB.a(
              ...
              Helper.getResString(R.string.design_manager_message_add_complete),
              0)
          .show();
      finish();
