...
public class ExportProjectActivity extends BaseAppCompatActivity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    getSupportActionBar()
        .setTitle(Helper.getResString(R.string.myprojects_export_project_actionbar_title));
    ...
    try {
      ...
      if (pathJava.exists()) {
        FileUtil.copyDirectory(
            pathJava,
            new File(
                project_metadata.javaFilesPath
                    + File.separator
                    + project_metadata.packageNameAsFolders));
      }
      ...
      if (pathNativeLibraries.exists()) {
        FileUtil.copyDirectory(
            pathNativeLibraries, new File(project_metadata.generatedFilesPath, "jniLibs"));
      }
      ...
      String exportedSourcesZipPath =
          wq.s() + File.separator + "export_src" + File.separator + exportedFilename;
      ...
      if (!new File(
              new FilePathUtil().getPathJava(sc_id) + File.separator + "SketchApplication.java")
          .exists()) {
      ...
      runOnUiThread(
          () -> {
            Log.e(
                "ProjectExporter",
                "While trying to export project's sources: " + e.getMessage(),
                e);
  ...
  private void initializeAppBundleExportViews() {
    export_aab_button.setOnClickListener(
        view -> {
          if (BuildConfig.FLAVOR.equals(BuildConfig.FLAVOR_NAME_WITHOUT_AABS)) {
            ...
            dialog.a(
                "This Sketchware Pro version doesn't support building AABs as it must work on "
                    + "Android 7.1.1 and earlier. Use Sketchware Pro "
                    + BuildConfig.VERSION_NAME_WITHOUT_FLAVOR
                    + "-"
                    + BuildConfig.FLAVOR_NAME_WITH_AABS
                    + " instead.");
            dialog.b(
                Helper.getResString(R.string.common_word_close),
  ...
  private void showAabSigningDialog() {
    GetKeyStoreCredentialsDialog credentialsDialog =
        new GetKeyStoreCredentialsDialog(
            this,
            R.drawable.ic_mtrl_key,
            "Sign outputted AAB",
            "Fill in the keystore details to sign the AAB.");
    credentialsDialog.setListener(
        credentials -> {
  ...
  private void initializeExportSrcViews() {
    export_source_loading_anim.setVisibility(View.GONE);
    export_source_output_stage.setVisibility(View.GONE);
    export_source_button.setOnClickListener(
        v -> {
  ...
  private void initializeSignApkViews() {
    ...
    sign_apk_button.setOnClickListener(
        view -> {
          aB confirmationDialog = new aB(this);
          confirmationDialog.b("Important note");
          confirmationDialog.a(
...
use a 3rd-party tool (for now).""");
  ...
  private void shareExportedSourceCode() {
    if (!export_src_filename.isEmpty()) {
      ...
      intent.putExtra(
          ...
          Helper.getResString(
              R.string.myprojects_export_src_title_email_subject, export_src_filename));
      intent.putExtra(
          Intent.EXTRA_TEXT,
          Helper.getResString(
              R.string.myprojects_export_src_title_email_body, export_src_filename));
      ...
      startActivity(
          Intent.createChooser(
              intent, Helper.getResString(R.string.myprojects_export_src_chooser_title_email)));
    }
  }
  ...
  private static class BuildingAsyncTask extends MA
      implements DialogInterface.OnCancelListener, BuildProgressReceiver {
      ...
      try {
        ...
        if (buildingAppBundle) {
          ...
          String createdBundlePath =
              AppBundleCompiler.getDefaultAppBundleOutputFile(project_metadata).getAbsolutePath();
          String signedAppBundleDirectoryPath =
          ...
          builder.runZipalign(
              builder.yq.unsignedUnalignedApkPath, builder.yq.unsignedAlignedApkPath);
          if (canceled) {
            cancel(true);
            return;
          }
        ...
        if (throwable instanceof LoadKeystoreException
          ...
          activity
              ...
              .runOnUiThread(
                  () ->
                      SketchwareUtil.showAnErrorOccurredDialog(
                          activity.get(),
                          "Either an incorrect password was entered, or your key store is"
                              + " corrupt."));
        } else {
          ...
          activity
              ...
              .runOnUiThread(
                  () ->
                      SketchwareUtil.showAnErrorOccurredDialog(
                          activity.get(), Log.getStackTraceString(throwable)));
        }
    ...
    public void a() {
      ...
      if (buildingAppBundle
          && new File(
                  ...
                  "sketchware" + File.separator + "signed_aab" + File.separator + aabFilename)
              .exists()) {
        ...
        dialog.a(
            "You can find the generated, signed AAB file at:\n"
                + "/Internal storage/sketchware/signed_aab/"
                + aabFilename);
        dialog.b(
            Helper.getResString(R.string.common_word_ok), Helper.getDialogDismissListener(dialog));
        dialog.show();
      }
    }

    /**
     * Called by a.a.a.MA if doInBackground() (a.a.a.MA#b()) returned a non-empty String, ergo, an
     * error occurred.
     */
    @Override // a.a.a.MA
     ...
     *
     * <p>If {@link #signWithTestkey} is <code>true</code> though, the result will be signed
     * regardless of {@link #configureResultJarSigning(String, char[], String, char[], String)} and
     * {@link #disableResultJarSigning()} calls.
     */
    public void configureResultJarSigning(
        String keystorePath,
    ...
    /**
     * Whether to sign the result with testkey or not. Note that this value will always be
     * prioritized over values set with {@link #configureResultJarSigning(String, char[], String,
     * char[], String)}.
     */
    ...
    /**
     * Disables JAR signing of the result. Equivalent to calling {@link
     * #configureResultJarSigning(String, char[], String, char[], String)} with <code>null</code>
     * parameters.
     *
     * <p>If {@link #signWithTestkey} is <code>true</code> though, the result will be signed
     * regardless of {@link #configureResultJarSigning(String, char[], String, char[], String)} and
    ...
    public boolean isResultJarSigningEnabled() {
      return signingKeystorePath != null
          && signingKeystorePassword != null
          && signingAliasName != null
          && signingAliasPassword != null
          && signingAlgorithm != null;
