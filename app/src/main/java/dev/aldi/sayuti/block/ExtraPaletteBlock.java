...
public class ExtraPaletteBlock {
  ...
  public boolean e(String str, String str2) {
    return switch (str) {
      ...
      case "onesignal" -> jC.a(sc_id).d(javaName, ComponentBean.COMPONENT_TYPE_ONESIGNAL, str2);
    ...
    for (int i = 0; i < customVariables.size(); i++) {
      ...
      if (split.length > 1) {
        ...
        SketchwareUtil.toastError(
            "Found invalid data of Custom Variable #"
                + (i + 1)
                + ": \""
                + customVariables.get(i)
                + "\"");
      }
    }
      ...
      if (component.type != 27) {
        logicEditor
            .a(
                ...
                "p",
                ComponentBean.getComponentTypeName(component.type),
                "getVar")
  ...
  private void blockCustomViews() {
    if (eventName.equals("onBindCustomView")) {
      ...
      if (viewBeanCustomView != null && !viewBeanCustomView.isEmpty()) {
        ArrayList<ViewBean> customViews =
            jC.a(sc_id).d(ProjectFileBean.getXmlName(viewBeanCustomView));
        for (int i = 0, customViewsSize = customViews.size(); i < customViewsSize; i++) {
          ...
          if (!customView.convert.equals("include")) {
            String typeName =
                customView.convert.isEmpty()
                    ? ViewBean.getViewTypeName(customView.type)
                    : IdGenerator.getLastPath(customView.convert);
            logicEditor
                .a(customView.id, "v", typeName, "getVar")
                .setTag(
                    isViewBindingEnabled
                        ? "binding." + ViewBindingBuilder.generateParameterFromId(customView.id)
                        : customView.id);
          }
        }
      }
      ...
      if (drawerViews != null) {
        for (int i = 0, drawerViewsSize = drawerViews.size(); i < drawerViewsSize; i++) {
          ...
          if (!drawerView.convert.equals("include")) {
            ...
            String typeName =
                drawerView.convert.isEmpty()
                    ? ViewBean.getViewTypeName(drawerView.type)
                    : IdGenerator.getLastPath(drawerView.convert);
            logicEditor
                .a(
                    isViewBindingEnabled
                        ? "binding.drawer." + ViewBindingBuilder.generateParameterFromId(id)
                        : id,
                    "v",
                    typeName,
                    "getVar")
  ...
  public void setBlock(int paletteId, int paletteColor) {
    ...
    switch (paletteId) {
      ...
      case 6:
        ...
        {
          boolean editTextUsed =
              isWidgetUsed("EditText") || extraBlocks.isCustomVarUsed("EditText");
          boolean textViewUsed =
              isWidgetUsed("TextView") || extraBlocks.isCustomVarUsed("TextView") || editTextUsed;
          boolean compoundButtonUsed =
              isWidgetUsed("CompoundButton")
          ...
          boolean autoCompleteTextViewUsed =
              isWidgetUsed("AutoCompleteTextView")
                  || extraBlocks.isCustomVarUsed("AutoCompleteTextView");
          boolean multiAutoCompleteTextViewUsed =
              isWidgetUsed("MultiAutoCompleteTextView")
                  || extraBlocks.isCustomVarUsed("MultiAutoCompleteTextView");
          boolean imageViewUsed =
              isWidgetUsed("ImageView")
                  || isWidgetUsed("CircleImageView")
                  || extraBlocks.isCustomVarUsed("ImageView");
          boolean ratingBarUsed =
              isWidgetUsed("RatingBar") || extraBlocks.isCustomVarUsed("RatingBar");
          boolean seekBarUsed = isWidgetUsed("SeekBar") || extraBlocks.isCustomVarUsed("SeekBar");
          boolean progressBarUsed =
              isWidgetUsed("ProgressBar") || extraBlocks.isCustomVarUsed("ProgressBar");
          boolean videoViewUsed =
              isWidgetUsed("VideoView") || extraBlocks.isCustomVarUsed("VideoView");
          boolean webViewUsed = isWidgetUsed("WebView") || extraBlocks.isCustomVarUsed("WebView");

          if (textViewUsed
              || compoundButtonUsed
              || autoCompleteTextViewUsed
              || multiAutoCompleteTextViewUsed
              || imageViewUsed
              || ratingBarUsed
              || seekBarUsed
              || progressBarUsed
        ...
        }
        {
          ...
          boolean gridViewUsed =
              isWidgetUsed("GridView") || extraBlocks.isCustomVarUsed("GridView");
        ...
        }
        {
          ...
          boolean textInputLayoutUsed =
              isWidgetUsed("TextInputLayout") || extraBlocks.isCustomVarUsed("TextInputLayout");

          if (drawerUsed
              || fabUsed
              || bottomNavigationViewUsed
              || swipeRefreshLayoutUsed
              || cardViewUsed
              || tabLayoutUsed
              || textInputLayoutUsed) {
      ...
      case 7:
        ...
        if (extraBlocks.isComponentUsed(ComponentBean.COMPONENT_TYPE_PROGRESS_DIALOG)
            ...
            || eventName.equals("onPreExecute")
            || eventName.equals("onProgressUpdate")
      ...
      case 8:
        ...
        if (ConfigActivity.isSettingEnabled(ConfigActivity.SETTING_SHOW_BUILT_IN_BLOCKS)) {
          ...
          logicEditor.addDeprecatedBlock(
              "Deprecated: Use XML Command Manager", "c", "CommandBlockXML");
