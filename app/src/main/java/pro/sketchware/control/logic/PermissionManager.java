...
public class PermissionManager {
  ...
  private void addReqPermission(
      boolean isAppCompat, ArrayList<String> checkPerm, ArrayList<String> reqPerm) {
    ...
    for (Entry<String, ArrayList<BlockBean>> blocks : jC.a(sc_id).b(javaName).entrySet()) {
      for (BlockBean block : blocks.getValue()) {
        if (block.opCode.equals("removePermission") && !block.parameters.get(0).trim().isEmpty()) {
          String permission =
              block.parameters.get(0).startsWith("Manifest")
                  ? block.parameters.get(0)
                  : ("Manifest.permission." + block.parameters.get(0));
    ...
    if (isAppCompat) {
      ...
      if (!checkPerm.isEmpty() && !addPerm.isEmpty()) {
        ...
        permissionCode.append(
            ") {" + Jx.EOL + "ActivityCompat.requestPermissions(this, new String[] {");
