...
public class LogicClickListener implements View.OnClickListener {
  ...
  private void addCustomVariable() {
    ...
    AddCustomVariableBinding binding =
        AddCustomVariableBinding.inflate(logicEditor.getLayoutInflater());

    binding.modifierLayout.setHelperText(
        "Enter modifier e.g. private, public, public static, or empty (package private).");

    VariableModifierValidator modifiersValidator =
        new VariableModifierValidator(getContext(), binding.modifierLayout);
    binding.modifier.addTextChangedListener(modifiersValidator);

    VariableTypeValidator varTypeValidator =
        new VariableTypeValidator(getContext(), binding.typeLayout);
    binding.type.addTextChangedListener(varTypeValidator);

    ZB validator =
        new ZB(getContext(), binding.nameLayout, uq.b, uq.a(), projectDataManager.a(projectFile));

    dialog.a(binding.getRoot());
    dialog.b(
        Helper.getResString(R.string.common_word_add),
        v -> {
  ...
  private void removeVariable() {
    ...
    RemoveAdapter adapter =
        new RemoveAdapter(
            ...
            data,
            variableName ->
                logicEditor.o.c(variableName)
                    || projectDataManager.c(javaName, variableName, eventName));
    recyclerView.setAdapter(adapter);

    List<Pair<List<Integer>, String>> variableTypes =
        List.of(
  ...
  private void addCustomList() {
    ...
    AddCustomListBinding listBinding =
        AddCustomListBinding.inflate(logicEditor.getLayoutInflater());

    ZB validator =
        new ZB(
            getContext(), listBinding.nameLayout, uq.b, uq.a(), projectDataManager.a(projectFile));
  ...
  private void removeList() {
    ...
    List<Pair<Integer, String>> listTypes =
        List.of(
  ...
  private static class RemoveAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private RemoveAdapter(
        Context context, List<Item> data, Function<String, Boolean> isInUseChecker) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
      if (viewType == Item.TYPE_TITLE) {
        ...
        textView.setPadding(dpToPx(2), dpToPx(4), dpToPx(4), dpToPx(4));
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
      ...
      if (viewType == Item.TYPE_TITLE) {
        ...
        checkBoxHolder.checkBox.setOnClickListener(
            v -> {
              ...
              if (item.type == Item.TYPE_ITEM && isChecked) {
                if (isInUseChecker.apply(item.text)) {
                  ...
                  SketchwareUtil.toastError(
                      Helper.getResString(item.inUseMessage), bB.TOAST_WARNING);
  ...
  private static class Item {
    ...
    @StringRes private final Integer inUseMessage;
