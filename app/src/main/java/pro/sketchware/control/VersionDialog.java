...
public class VersionDialog {
  ...
  public void show() {
    ...
    binding.versionCode.setText(
        String.valueOf(Integer.parseInt(activity.binding.verCode.getText().toString())));
    ...
    dialog.b(
        activity.getString(R.string.common_word_save),
        v -> {
          ...
          if (!mB.a() && validVerCode && validVerName) {
            ...
            activity.binding.verName.setText(
                !verNamePostfix.isEmpty() ? (verName + " " + verNamePostfix) : verName);
            dialog.dismiss();
          }
        });

    binding.versionName2.addTextChangedListener(
        new VersionNamePostfixValidator(activity, binding.tilVersionNameExtra));
    dialog.a(
        activity.getString(R.string.common_word_cancel), Helper.getDialogDismissListener(dialog));
    dialog.show();
  }
}
