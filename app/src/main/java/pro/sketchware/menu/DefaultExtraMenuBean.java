...
public class DefaultExtraMenuBean {
  ...
  public Pair<String, ArrayList<String>> getMenu(Ss menu) {
    ...
    switch (menuName) {
      ...
      // This is meant to be a built-in menu including the cases below, but Aldi implemented it as a
      // file, which is why, in some cases, certain menus appear empty.
      // start
      ...
      case "image" -> {
        ...
        for (String drawable_xhdpi : FileUtil.listFiles(path, "")) {
          if (drawable_xhdpi.contains(".png") || drawable_xhdpi.contains(".jpg")) {
            menus.add(
                getFilename(drawable_xhdpi, drawable_xhdpi.contains(".png") ? ".png" : ".jpg"));
          }
        }
      }
      case "til_box_mode" -> {
