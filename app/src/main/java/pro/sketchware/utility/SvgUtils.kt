...
class SvgUtils(private val context: Context) {
  ...
  fun initImageLoader() {
    imageLoader = ImageLoader.Builder(context).components { add(SvgDecoder.Factory()) }.build()
  }

  fun loadImage(imageView: ImageView, filePath: String) {
    val file = File(filePath)
    if (file.exists()) {
      val request: ImageRequest = ImageRequest.Builder(context).data(file).target(imageView).build()
      ...
      ImageRequest.Builder(context)
        ...
        .target { drawable ->
          drawable.let {
            ...
            val scaledBitmap =
              Bitmap.createScaledBitmap(bitmap, 48 * densityScale, 48 * densityScale, true)
  ...
  private fun hasFillAttribute(parser: XmlPullParser): Boolean {
    return (0 until parser.attributeCount).any { parser.getAttributeName(it) == "fill" }
  }
