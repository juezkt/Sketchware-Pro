...
public class WidgetsCreatorManager {
  ...
  private final String widgetResourcesDirectoryPath =
      "/storage/emulated/0/.sketchware/resources/widgets/";
  ...
  private final List<String> availableWidgetsNames =
      Arrays.asList(
  ...
  private boolean isInvalidWidget(HashMap<String, Object> widgetData) {
    List<String> requiredKeys =
        Arrays.asList("Class", "title", "name", "inject", "type", "position");
  ...
  public void showWidgetsCreatorDialog(int position) {
    ...
    dialog.b(
        isEditing
            ? Helper.getResString(R.string.widget_editor)
            : Helper.getResString(R.string.create_new_widget));
    WidgetsCreatorDialogBinding binding =
        WidgetsCreatorDialogBinding.inflate(LayoutInflater.from(context));
    ...
    if (isEditing) {
      ...
      dialog.configureDefaultButton(
          Helper.getResString(R.string.common_word_see_more), view -> showMorePopUp(dialog, view));
    }
    ...
    binding.widgetType.setOnClickListener(
        v ->
            showTypeViewSelectorDialog(
                availableWidgetsNames, availableWidgetsTypes, binding.widgetType));
    binding.addWidgetTo.setOnClickListener(
        v -> {
    ...
    dialog.b(
        ...
        v -> {
          try {
            String widgetTitle =
                Objects.requireNonNull(binding.widgetTitle.getText()).toString().trim();
            String widgetName =
                Objects.requireNonNull(binding.widgetName.getText()).toString().trim();
            String widgetType =
                Objects.requireNonNull(binding.widgetType.getText()).toString().trim();
            String widgetInject =
                Objects.requireNonNull(binding.injectCode.getText()).toString().trim();
            String widgetClass =
                Objects.requireNonNull(binding.addWidgetTo.getText()).toString().trim();

            if (widgetTitle.isEmpty()) {
              binding.inputTitle.setError(
                  String.format(Helper.getResString(R.string.var_is_required, "Widget title")));
              return;
            }
            if (widgetName.isEmpty()) {
              binding.inputName.setError(
                  String.format(
                      Helper.getResString(R.string.var_is_required), "Widget class name"));
              return;
            }
            if (widgetType.isEmpty()) {
              binding.inputType.setError(
                  String.format(Helper.getResString(R.string.var_is_required), "Widget type"));
              return;
            }
            if (widgetClass.isEmpty()) {
              binding.inputClass.setError(
                  String.format(Helper.getResString(R.string.var_is_required), "Category"));
              return;
            }
        ...
        menuItem -> {
          ...
          if (menuItem.getItemId() == R.id.import_widgets) {
            ...
            FilePickerDialog pickerDialog =
                new FilePickerDialog(context, properties, R.style.RoundedCornersDialog);
  ...
  public static void clearErrorOnTextChanged(
  ...
  private void showTypeViewSelectorDialog(
      List<String> choices, List<String> types, TextInputEditText type) {
    AtomicInteger choice = new AtomicInteger();
    new MaterialAlertDialogBuilder(context)
        .setTitle(Helper.getResString(R.string.widget_type_title))
        .setSingleChoiceItems(
            choices.toArray(new String[0]),
            types.indexOf(Objects.requireNonNull(type.getText()).toString()),
            (dialog2, which) -> choice.set(which))
        .setPositiveButton(
            R.string.common_word_save, (dialog2, which) -> type.setText(types.get(choice.get())))
  ...
  private void showCategorySelectorDialog(List<String> choices, TextInputEditText type) {
    AtomicInteger choice =
        new AtomicInteger(choices.indexOf(Objects.requireNonNull(type.getText()).toString()));
    ...
    ArrayAdapter<String> adapter =
        new ArrayAdapter<>(context, android.R.layout.simple_list_item_single_choice, choices);
    ...
    if (!(choice.get() >= 0
        && choice.get() < choices.size()
        && choices.get(choice.get()).equals(type.getText().toString()))) {
      newEditText.setText(type.getText().toString());
    }

    builder
        .setTitle(Helper.getResString(R.string.add_to))
        .setView(layout)
        .setPositiveButton(
    ...
    newEditText.addTextChangedListener(
        new TextWatcher() {
          @Override
          public void beforeTextChanged(CharSequence s, int start, int count, int after) {}
  ...
  public void showActionsDialog(int tag) {
    ...
    DialogSelectorActionsBinding dialogBinding =
        DialogSelectorActionsBinding.inflate(LayoutInflater.from(activity));
    ...
    dialogBinding.export.setOnClickListener(
        v -> {
  ...
  private void deleteWidgetMap(int position) {
    ...
    dialog.b(
        xB.b().a(context, R.string.common_word_delete),
        v -> {
          String Class =
              Objects.requireNonNull(widgetConfigurationsList.get(position).get("Class"))
                  .toString();
    ...
    dialog.a(
        xB.b().a(context, R.string.common_word_cancel), Helper.getDialogDismissListener(dialog));
    dialog.show();
  }
