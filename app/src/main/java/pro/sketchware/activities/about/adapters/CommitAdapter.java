...
public class CommitAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
  ...
  public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    if (viewType == TYPE_LOADING) {
      AboutCommitAdapterItemProgressBinding binding =
          AboutCommitAdapterItemProgressBinding.inflate(
              LayoutInflater.from(parent.getContext()), parent, false);
      return new LoadingViewHolder(binding);
    } else {
      AboutBetaChangesBinding binding =
          AboutBetaChangesBinding.inflate(LayoutInflater.from(parent.getContext()), parent, false);
    ...
    if (holder instanceof ItemViewHolder itemHolder) {
      ...
      if (isCoreTeamMember(commitDetails.getCommitterName())) {
        ...
        activeBackgroundColor =
            MaterialColors.getColor(itemHolder.binding.tvStatus, R.attr.colorCoolGreenContainer);
        activeBackgroundTextColor =
            MaterialColors.getColor(itemHolder.binding.tvStatus, R.attr.colorOnCoolGreenContainer);
      } else {
        itemHolder.binding.tvStatus.setText("Contributor");
        activeBackgroundColor =
            MaterialColors.getColor(itemHolder.binding.tvStatus, R.attr.colorAmberContainer);
        activeBackgroundTextColor =
            MaterialColors.getColor(itemHolder.binding.tvStatus, R.attr.colorOnAmberContainer);
      }
      ...
      if (shaPosition > position || shaPosition == -1) {
        ...
        activeBackgroundColor =
            MaterialColors.getColor(
                itemHolder.binding.updateStatus, R.attr.colorCoolGreenContainer);
        activeBackgroundTextColor =
            MaterialColors.getColor(
                itemHolder.binding.updateStatus, R.attr.colorOnCoolGreenContainer);
        advancedCorners(
            itemHolder.binding.viewLeftline,
            MaterialColors.getColor(itemHolder.binding.viewLeftline, R.attr.colorGreen));
      } else if (shaPosition < position) {
        itemHolder.binding.updateStatus.setText("Old Version");
        activeBackgroundColor =
            MaterialColors.getColor(itemHolder.binding.updateStatus, R.attr.colorVioletContainer);
        activeBackgroundTextColor =
            MaterialColors.getColor(itemHolder.binding.updateStatus, R.attr.colorViolet);
        advancedCorners(
            itemHolder.binding.viewLeftline,
            MaterialColors.getColor(itemHolder.binding.viewLeftline, R.attr.colorViolet));
      } else {
        itemHolder.binding.updateStatus.setText("Current Version");
        activeBackgroundColor =
            MaterialColors.getColor(itemHolder.binding.updateStatus, R.attr.colorAmberContainer);
        activeBackgroundTextColor =
            MaterialColors.getColor(itemHolder.binding.updateStatus, R.attr.colorOnAmberContainer);
        advancedCorners(
            itemHolder.binding.viewLeftline,
            MaterialColors.getColor(itemHolder.binding.viewLeftline, R.attr.colorAmber));
      }
      rippleRound(
          itemHolder.binding.updateStatus, activeBackgroundColor, activeBackgroundColor, 100);
