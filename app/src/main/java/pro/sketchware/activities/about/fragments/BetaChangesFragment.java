...
public class BetaChangesFragment extends Fragment {
  ...
  private void loadCommitData(int page) {
    ...
    String commitsUrl =
        String.format(
            Helper.getResString(R.string.link_github_commits_url),
      ...
      new Network()
          .get(
              ...
              response -> {
                ...
                if (response != null) {
                  Gson gson = new Gson();
                  AboutResponseModel.CommitDetails[] commitDetailsArray =
                      gson.fromJson(response, AboutResponseModel.CommitDetails[].class);
                  if (commitDetailsArray != null) {
                    aboutAppData.setCommitDetailsList(
                        new ArrayList<>(Arrays.asList(commitDetailsArray)));
                    nextPage++;
                  }
                }
              });
