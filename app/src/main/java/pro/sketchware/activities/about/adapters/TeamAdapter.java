...
public class TeamAdapter extends RecyclerView.Adapter<TeamAdapter.ViewHolder> {
  ...
  public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    AboutTeamviewBinding binding =
        AboutTeamviewBinding.inflate(LayoutInflater.from(parent.getContext()), parent, false);
  ...
  public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
    ...
    boolean isTitled =
        position == 0 || !(member.getTitle().equals(team.get(position - 1).getTitle()));
    if (isTitled) {
    ...
    if (member.isCoreTeamMember()) {
      advancedCorners(
          holder.binding.viewLeftline,
          MaterialColors.getColor(
              holder.binding.viewLeftline, com.google.android.material.R.attr.colorPrimary));
    } else {
      advancedCorners(
          holder.binding.viewLeftline,
          MaterialColors.getColor(holder.binding.viewLeftline, R.attr.colorGreen));
    }
    ...
    if (member.isActive()) {
      ...
      activeBackgroundColor =
          MaterialColors.getColor(holder.binding.tvStatus, R.attr.colorCoolGreenContainer);
      activeBackgroundTextColor =
          MaterialColors.getColor(holder.binding.tvStatus, R.attr.colorOnCoolGreenContainer);
    } else {
      holder.binding.tvStatus.setText("Inactive");
      activeBackgroundColor =
          MaterialColors.getColor(holder.binding.tvStatus, R.attr.colorAmberContainer);
      activeBackgroundTextColor =
          MaterialColors.getColor(holder.binding.tvStatus, R.attr.colorOnAmberContainer);
    }
    ...
    holder.binding.memberLayout.setOnClickListener(
        view -> openMemberGithubProfile(view.getContext(), member.getMemberUsername()));
  }
