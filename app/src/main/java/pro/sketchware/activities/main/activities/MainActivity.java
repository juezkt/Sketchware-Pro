...
public class MainActivity extends BasePermissionAppCompatActivity {
  private final OnBackPressedCallback closeDrawer =
  ...
  public void g(int i) {
    if (i == 9501) {
      ...
      ProjectsFragment projectsFragment =
          fragmentsAdapter != null ? fragmentsAdapter.getProjectsFragment() : null;
      ...
      switch (requestCode) {
        ...
        case 212:
          if (!(data.getStringExtra("save_as_new_id") == null
                      ? ""
                      : data.getStringExtra("save_as_new_id"))
                  .isEmpty()
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    drawerToggle =
        new ActionBarDrawerToggle(this, binding.drawerLayout, R.string.app_name, R.string.app_name);
    ...
    if (Intent.ACTION_VIEW.equals(getIntent().getAction())) {
      ...
      if (data != null) {
        new SingleCopyTask(
                this,
                new SingleCopyTask.CallBackTask() {
                  @Override
                    ...
                    if (wasSuccessful) {
                      ...
                      if (BackupFactory.zipContainsFile(path, "local_libs")) {
                        new MaterialAlertDialogBuilder(MainActivity.this)
                            .setTitle("Warning")
                            .setMessage(
                                BackupRestoreManager.getRestoreIntegratedLocalLibrariesMessage(
                                    false, -1, -1, null))
                            .setPositiveButton(
                                "Copy", (dialog, which) -> manager.doRestore(path, true))
                            .setNegativeButton(
                                "Don't copy", (dialog, which) -> manager.doRestore(path, false))
  ...
  private BottomSheetDialogView getBottomSheetDialogView() {
    ...
    bottomSheetDialog.setPositiveButton(
        "View changes",
        (dialog, which) -> {
  ...
  private void allFilesAccessCheck() {
    if (Build.VERSION.SDK_INT > 29) {
      ...
      if (!optOutFile.exists() && !granted) {
        ...
        dialog.a(
            "Starting with Android 11, Sketchware Pro needs a new permission to avoid "
                + "taking ages to build projects. Don't worry, we can't do more to storage than "
                + "with current granted permissions.");
        dialog.b(
            Helper.getResString(R.string.common_word_settings),
            v -> {
              ...
              try {
                ...
                Log.e(
                    ...
                    "Error while trying to create "
                        + "\"Don't show Android 11 hint\" dialog file: "
                        + e.getMessage(),
                    e);
              }
              dialog.dismiss();
