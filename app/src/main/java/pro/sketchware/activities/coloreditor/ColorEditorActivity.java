...
public class ColorEditorActivity extends AppCompatActivity {
  ...
  private static String getColorValueFromXml(
      Context context, String colorName, int referencingLimit) {
    try {
  ...
  private void showExitDialog() {
    ...
    dialog.a(
        xB.b().a(activity, R.string.common_word_exit),
        v -> {
          dialog.dismiss();
          finish();
        });
      ...
      while (eventType != XmlPullParser.END_DOCUMENT) {
        ...
        switch (eventType) {
          ...
          case XmlPullParser.END_TAG:
            if ("color".equals(tagName)) {
              if ((colorName != null)
                  && PropertiesUtil.isHexColor(
                      getColorValue(getApplicationContext(), colorValue, 4))) {
  ...
  public void showColorEditDialog(ColorItem colorItem, int position) {
    ...
    XB colorValidator =
        new XB(this, dialogBinding.colorValueInputLayout, dialogBinding.colorPreview);

    if (colorItem != null) {
      dialogBinding.colorKeyInput.setText(colorItem.getColorName());
      dialogBinding.colorPreview.setBackgroundColor(
          PropertiesUtil.parseColor(
              getColorValue(activity.getApplicationContext(), colorItem.getColorValue(), 3)));
    ...
    dialogBinding.colorPreviewCard.setOnClickListener(
        v -> {
          colorpicker.a(
              new Zx.b() {
                @Override
                public void a(int colorInt) {
                  ...
                  dialogBinding.colorPreviewCard.setCardBackgroundColor(
                      PropertiesUtil.parseColor(selectedColorHex));
