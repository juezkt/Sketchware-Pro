...
public class EventsManagerFragment extends qA {
  ...
  public static String getNumOfEvents(String name) {
    ...
    if (FileUtil.isExistFile(EventsManagerConstants.EVENTS_FILE.getAbsolutePath())) {
      ArrayList<HashMap<String, Object>> events =
          getGson()
              .fromJson(
                  FileUtil.readFile(EventsManagerConstants.EVENTS_FILE.getAbsolutePath()),
  ...
  public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
    ...
    binding.activityEvents.setOnClickListener(
        v -> openFragment(new EventsManagerDetailsFragment()));
  ...
  private void showListenerDialog(
      @Nullable HashMap<String, Object> existingListener, int position) {
    var listenerBinding =
        DialogAddNewListenerBinding.inflate(LayoutInflater.from(requireContext()));
      ...
      if ("true".equals(existingListener.get("s"))) {
        ...
        listenerBinding.listenerCode.setText(
            existingListener
                .get("code")
                .toString()
                .replaceFirst("//" + listenerBinding.listenerName.getText().toString() + "\n", ""));
      }
    }
                ...
                (di, i) -> {
                  ...
                  if (!listenerName.isEmpty()) {
                    HashMap<String, Object> hashMap =
                        existingListener != null ? existingListener : new HashMap<>();
                    hashMap.put("name", listenerName);
                    hashMap.put(
                        "code",
                        listenerBinding.listenerIsIndependentClassOrMethod.isChecked()
                            ? "//"
                                + listenerName
                                + "\n"
                    ...
                    hashMap.put(
                        ...
                        listenerBinding.listenerIsIndependentClassOrMethod.isChecked()
                            ...
                            : "false");
                    hashMap.put(
                        "imports", listenerBinding.listenerCustomImport.getText().toString());
                    if (position >= 0) {
  ...
  public void refreshList() {
    ...
    if (FileUtil.isExistFile(EventsManagerConstants.LISTENERS_FILE.getAbsolutePath())) {
      listMap =
          getGson()
              .fromJson(
                  FileUtil.readFile(EventsManagerConstants.LISTENERS_FILE.getAbsolutePath()),
                  Helper.TYPE_MAP_LIST);
  ...
  private void showImportEventsDialog() {
    ...
    FilePickerDialog filePickerDialog =
        new FilePickerDialog(requireContext(), dialogProperties, R.style.RoundedCornersDialog);
        ...
        selections -> {
          if (FileUtil.readFile(selections[0]).isEmpty()) {
            ...
            try {
              ...
              importEvents(
                  getGson().fromJson(split[0], Helper.TYPE_MAP_LIST),
  ...
  private void importEvents(
      ArrayList<HashMap<String, Object>> data, ArrayList<HashMap<String, Object>> data2) {
    ...
    events.addAll(data2);
    FileUtil.writeFile(
        EventsManagerConstants.EVENTS_FILE.getAbsolutePath(), getGson().toJson(events));
    listMap.addAll(data);
    FileUtil.writeFile(
        EventsManagerConstants.LISTENERS_FILE.getAbsolutePath(), getGson().toJson(listMap));
    refreshList();
  ...
  private void exportListener(int p) {
    String concat =
        FileUtil.getExternalStorageDir().concat("/.sketchware/data/system/export/events/");
    ...
    FileUtil.writeFile(
        concat + ex.get(0).get("name").toString() + ".txt",
        getGson().toJson(ex) + "\n" + getGson().toJson(ex2));
  ...
  private void exportAllEvents() {
    ...
    FileUtil.writeFile(
        new File(EventsManagerConstants.EVENT_EXPORT_LOCATION, "All_Events.txt").getAbsolutePath(),
  ...
  public class ListenersAdapter extends RecyclerView.Adapter<ListenersAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
      LayoutEventItemBinding binding =
          LayoutEventItemBinding.inflate(LayoutInflater.from(parent.getContext()), parent, false);
